...
After committing the exposure/tooling patch, a second stack-sync-off capture was
run on the same installed APK to turn the first comparison into an
experiment-control-experiment sandwich.

Repeat experiment, stack sync off:

- Capture: `scratch/thor-debug/20260524-021858-*`
- HEAD: `7f271cb5c`
- APK SHA-256:
  `5FD5488026AA7152ED86E352864EC7FA160BE9ADBE1E0D4B7B2827FF554E85CF`
- Cvar: `a64_enable_host_guest_stack_synchronization=false`
- Route: visible Blue Dragon opening sky/wing scene
- Fatal-marker search: clean
- Final `82282490`: `body_ticks_total=27192157`,
  final interval `ticks_per_entry=146`, `code_size=85104`
- Final `82281D28`: `body_ticks_total=7809349`,
  final interval `ticks_per_entry=88`, `code_size=94020`
- Final perf sample: Main Thread `96.1%`, GPU Commands `11.5%`,
  XMA Decoder `42.3%`, Draw Thread `3.8%`

## Read

Turning stack synchronization off is route-clean in two same-APK opening-route
captures and deterministically shrinks generated code for the filtered
functions:

- `82282490` code size: `87168` -> `85104`.
- `82281D28` code size: `96908` -> `94020`.

The body-time result is mixed, not a speed proof. The first stack-sync-off run
looked modestly better than the stack-sync-on control
(`27192906` -> `26462740` for `82282490`), but the repeat stack-sync-off run
landed at `27192157`, essentially matching the stack-sync-on total while still
keeping the smaller code size. Treat the first lower total as route/timing
variance until a more direct profiler proves otherwise.

This is not enough to promote global or Blue Dragon preset stack
synchronization off. The flag guards the imported A64 stackpoint/host-guest
stack path, so a broad default-off change could break longjmp/setjmp,
exception, or unwind-sensitive behavior outside this route.
...
- Treat stack-sync-off as code-size evidence only until a direct stackpoint or
  prolog profiler proves a repeatable speed win.
...
Do not run another unchanged stack-sync A/B. Add lower-risk stackpoint/prolog
overhead attribution before changing the Blue Dragon speed pack, or continue
with `822825E0 -> 82282490` recursive-call/prolog cost reduction. Do not return
to stale VMX/vector, r1 live-in, or call-edge-only probes without fresh body-time
evidence.
