# ARCHIVE: the BD EDRAM / D3D9-HLE era (2026-07)

Moved out of CLAUDE.md on 2026-08-07. **Superseded** by the measurement that BD's
field is CPU-bound, not GPU-bound - see the section of that name in CLAUDE.md.

Kept verbatim rather than summarised, because most of its value is negative: it
records what was tried and killed WITH DATA, and re-deriving any of it costs
device time that has already been spent. Do not reopen an item here without
reading why it died.
