...
| **BIN-ONCE at the D3D source (EndTiling HLE)** — `device+0x30b8=1` + tile-0=full-surface | collapse BD's 2-tile field to 1 pass → halve the per-tile-replayed vertex/binning | **RE-CONFIRMED 2026-07-04 the field IS 2-tiled + ALL draws re-submitted per tile (INDIRECT_BUFFER x N via the 0x88 tile-loop-back in FUN_82487CC0); predication only CULLS, vertex work still N×**. 360 needs 2 tiles (720p+MSAA ~15MB > 10MB EDRAM); THOR's emulated EDRAM has no limit → render once. Recipe: force device+0x30b8=1 + tile-0 rect(device+0x30bc)=full surface + EDRAM base(device+0x31ac / resolve srcs +0x32a0/+0x32b4)=full-surface base + window-offset=0, COHERENTLY at EndTiling FUN_8248A680 (the count=1 fast-path @0x8248a978 already exists in the guest). | **OPEN — BUILDING** (the real BD-field win; halves foliage vertex cost) |
