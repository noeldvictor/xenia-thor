...
5. The object cache is keyed on a content hash of the lowering sources
   (`build/llvm_lowering_stamp.h`, generated by `tools/build/gen_version_h.py`). A rebuild that does
   not change the lowering keeps every title's cache. Before 2026-09-20 the key was the compile time
   and every rebuild cost a cold compile per title.
6. When the PC can answer, use the PC: `xenia.exe --storage_root=<scratch> --log_file=<file> <iso>`
   boots Blue Dragon in seconds and prints the same kernel log lines.
7. Name the title from the log ("Title name:") or the crash report before you attribute a crash.
8. Files in this tree mix CRLF and LF, and the Bash tool's heredoc halves backslashes. Patch files
   with a script written to the scratchpad, normalize line endings inside it, and keep the size gate.
9. The crash diagnostic names a64 functions by range and LLVM functions by the nearest function
