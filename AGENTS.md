...
- Latest reset note:
  ...
  The SurfaceFlinger capture proved bad game `SurfaceView` present cadence
  (`126` valid presents over `55.150s`, average interval `441.196ms`, max
  `2361.047ms`) while Main Thread / A64 remains the wall, but the old capture
  lacks device monotonic sampler timestamps and cannot be time-joined. The
  sampler now records `device_monotonic_ns`, and the new attribution tool fails
  closed on old logs. The next useful runtime slice is a fresh route capture
  with the timestamped sampler, followed by the proof summary, route report,
  and present/jank attribution report. Do not reopen closed A64 micro-lanes or
  patch kernel/HLE behavior from aggregate latency evidence alone.
...
- A64 backend maturity: register allocation, guest-state cache design, helper
  ...
  fatal-marker search. `docs/research/20260526-075500-a64-offset-address-fastpath-quiet-ab.md`
  records the same-APK ON/OFF/ON sandwich on commit `5ffe7d20b` / the same APK
  SHA. The cvar is route-clean and locally positive for generated-code/body-time
  but not full-speed proof: `82282490` code size `87168 -> 86452` with both ON
  captures below OFF body ticks, `82281D28` code size `96908 -> 95724` with both
  ON captures below OFF body ticks, and `82287788` code size `35732 -> 35712`
  with small matching movement. Main Thread remains near a full core and no
  ...
  pressure. `docs/research/20260526-090500-a64-nonclosed-gpr-cache-audit-skeleton.md`
  added the default-off, counter-only post-promotion audit, and
  `docs/research/20260526-092500-a64-nonclosed-gpr-cache-capture.md` captured
  it route-clean for `0x82281D28`. That capture closes current non-closed GPR
  cache behavior: `candidate_loads=181`, `candidate_stores=75`, but
