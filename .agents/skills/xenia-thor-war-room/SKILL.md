...
description: Coordinate repo-local Xbox 360 PowerPC, AArch64 Snapdragon JIT, Vulkan/Adreno, Android, Ghidra, RenderDoc, ADB, speed profiling, experiment-gate, and Blue Dragon route evidence lanes for the experimental xenia-thor bring-up. Use for broad "get Blue Dragon farther" tasks, unclear crashes, black screens, watchdogs, slow one-at-a-time debugging, or when multiple expert perspectives must be combined before editing code.
---
...
- AYN Thor Max ARM64 is the priority target; Windows is a control lane only
