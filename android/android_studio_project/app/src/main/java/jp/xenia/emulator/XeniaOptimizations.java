...
public final class XeniaOptimizations {
    ...
    static {
        ...
        list.add(new Optimization(
                "opt_aot_object_cache",
                "Keep compiled code between launches (object cache)",
                "Saves each LLVM-compiled function to disk, so the next launch "
                        + "of the same game loads it instead of compiling again.",
                "A cold LLVM compile of a whole game runs single-threaded and "
                        + "takes minutes. With the cache, the second launch of a "
                        + "title skips that work. The cache lives in the app's "
                        + "files/objcache directory and is rebuilt after an app "
                        + "update. Turn off only to force a full recompile.",
                CATEGORY_CPU, true, true,
                new BoolCvar[]{
                        new BoolCvar("cpu_llvm_object_cache"),
                        new BoolCvar("cpu_llvm_object_cache_skip_lowering")}, null));
    ...
    public static void applyTo(
        ...
        for (final Optimization opt : ALL) {
            if (!opt.isEnabledForGame(prefs, titleId)) {
                // The menu is the control surface both ways (directive 17): an
                // off toggle writes its bool cvars false, so a native default
                // that is on cannot override the user's choice. Int cvars
