...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260524-011500-82485dd8-word-copy-loop-fastpath.md. The default-off arm64_blue_dragon_word_copy_loop_fastpath experiment for 82485DD8:82485E70 is route-clean and speed-positive in the profiled same-APK lane: fastpath-on scratch/thor-debug/20260524-010729-* lowered 82485DD8 body_ticks_total to 695645 and 82485E70 block body-time to 75310, while fastpath-off control scratch/thor-debug/20260524-011110-* had 82485DD8=2307804 and 82485E70=1122726. Keep it default-off until a quiet same-APK A/B without block body-time/disassembly confirms route-wide speed. " +
            ...
            "Next slice should run a quiet same-APK A/B for arm64_blue_dragon_word_copy_loop_fastpath=false then true with body-time comparators 82490030,82486178,82485DD8,82486018, no disassembly dump, and no block body-time profiler. Keep call-edge/profiling cvars default-off outside targeted captures."
        )
