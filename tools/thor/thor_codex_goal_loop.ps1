...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lane: the compile-time route-ranking audit reports decision=rank_context_cr_scalar_state_source_audit_next. Run a source-only A64 context / CR compare / branch lowering audit for mapped body-dominant blocks 82282490:822825E0-822825F0, 82281D28:8228233C-82282374, and 82287788:822877BC-82287864. Do not patch behavior, run Thor speed A/B, add another fast-entry micro-audit, use host-pointer immediate fastmem without no-wrap proof, reopen closed VMX128/nonclosed-cache behavior, or fuse single-PC barriers from this ranking alone. A64Function fast-entry source contracts exist but generated behavior still does not use them; fast-entry stays closed until guard emission, payload population, dirty flush, late-bound fallback, stackpoint resume, and debug/exception visibility are solved explicitly. Use -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but keep the cvar default-off globally. GPU offload only when counters justify it; Android controller/settings UX is separate playability work. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
