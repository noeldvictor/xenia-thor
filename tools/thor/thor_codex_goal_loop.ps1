...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lane: the best-current quiet route proof refresh is complete and reports sustained_30fps_proof=false, proof_reason=no_fps_markers, Main Thread 96.1%, GPU Commands 3.8%, and the same 82282490/82281D28/82287788 A64 body-time wall. Do not rerun that capture unchanged and do not reopen current A64/VMX/fastmem/helper/GPU behavior lanes from it. The next automatic slice needs a genuinely new structural class with fresh evidence outside the closed wall cluster, or a user-prioritized Android controller/settings UX lane. If no such lane is named, write a blocker instead of launching another Thor speed capture or A64 micro-audit. Keep -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but keep the cvar default-off globally. Do not use -AllowOrdinalFallback for behavior decisions.",
