...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260524-021116-82282490-recursive-stack-sync-probe.md. Fresh route-clean block/call-edge captures under the Blue Dragon word-loop speed preset confirm the current wall is recursive 82282490 work, not stale 822824F0 vector work. Dynamic call-edge rows put 822825E0 -> 82282490 first with body_ticks_total=28602334, calls_total=117425, and peak ticks_per_call=1244; 822825C8 -> 8227FEE8 is secondary with body_ticks_total=11951162. Android/Thor tooling now exposes a64_enable_host_guest_stack_synchronization, but keep the Blue Dragon speed-pack default true. Stack-sync-off was route-clean twice and shrank generated code for 82282490 from 87168 to 85104, but the repeat off capture landed at 82282490=27192157, essentially matching the stack-sync-on control at 27192906. Treat this as code-size evidence only, not speed proof. Do not run another unchanged stack-sync A/B; next add stackpoint/prolog overhead attribution or directly reduce recursive call/prolog cost. " +
