...
function New-AndroidUsabilityPrompt {
    $parts = @(
        ...
        "Known state: Project Sylpheed title/menu geometry, Android reenter abort, pthread_join abort, and XMA decoder abort have fixes/proofs. Android now has launcher-path AAudio driver-open proof; by-ear audible-output confirmation still needs the user present. Project Sylpheed still exposes heap release failures without a scoped crash marker. Recent-game status, controller remap, Back OSD, FPS badge, Exit to menu, and internal resolution settings exist.",
        "Preferred next product slice: continue Project Sylpheed heap ownership compatibility, confirm audible output by ear with the user present, or handle another user-reported Android app usability bug.",
