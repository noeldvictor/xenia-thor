...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision notes: docs/research/20260525-203000-continual-harness-plan-reset.md, docs/research/20260525-203900-a64-mature-gap-reset-rerank.md, docs/research/20260526-012000-vmx128-route-stabilized-counters.md, docs/research/20260526-014000-82281d28-hir-block-profile-join.md, and docs/research/20260526-015900-a64-block-profile-metadata-mapper.md. Use them as the active plan if present.",
        "Closed lanes: do not repeat f1/edge-payload behavior, stvewx, three-PC MUL_ADD_V128, fpscr, pre/post-promotion r1/r11, local-only 822824F0, broad Vulkan work, PACK/UNPACK VMX128 behavior, or broad PERMUTE/LOAD_VECTOR_SHL/SHR behavior without fresh body-dominant evidence. The edge-payload lifetime audit found segments_survived_no_kill=0 and f1_reads_before_kill=0, so do not materialize payload state or run a quiet speed A/B from that lane. PACK/UNPACK route-volume found PACK absent and UNPACK zero weighted hot-block volume. The 20260526 VMX128 route counters found current vector volume either in closed local stvewx/vmaddfp shapes or in smaller non-dominant 82287788 blocks. The 82281D28 HIR/block-profile join audit found join_status=unsafe; the metadata mapper patch exists, but do not patch behavior from 82281D28 weighted HIR joins until a fresh metadata capture proves the mapping.",
        ...
        "Preferred next lanes are structural and currently ranked: FullDeploy plus a route-clean one-function 82281D28 block-profile metadata capture, then run tools/thor/thor_hir_block_profile_join_audit.ps1 on the new log and require metadata-backed mapping; then guest-state cache design only if it avoids closed r1/r11/f1/fpscr/barrier lanes; then helper/block-linking only if static audit proves a reusable direct-call or stackpoint rule beyond closed stack-sync and edge-payload lanes; then fastmem only with counter evidence; and GPU offload only when counters justify it. Do not use -AllowOrdinalFallback for behavior decisions.",
