...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-124029-8227fee8-callee-call-edge-split.md. Route-clean capture scratch/thor-debug/20260523-123406-* reached the visible opening sky/wing route on APK SHA 962D3086F4030D9BD5A9D46AF5E8DFA4A320A13BFCD14135B8B077AECDC31CC5 with a clean fatal-marker search. It split the prior 8227FEE8 call-heavy wall: final body-time put 8227F1D8 first in the last interval with body_ticks_total=3714635, body_ticks_delta=844204, ticks_per_entry=84, code_size=20180, while 8247BE20 was tiny at body_ticks_total=27747, ticks_per_entry=2, code_size=796. Dynamic call-edge rows identify 822809F4 -> 8227F1D8 as the dominant child edge: calls_total=26098, body_ticks_total=2031295, peak_delta=1137492, peak_ticks_per_call=216. " +
            "Previous route-clean filtered parent HIR remains docs/research/20260522-234847-8227fee8-filtered-hir-sandwich.md, and owner-attribution remains documented in docs/research/20260523-001018-a64-owner-thread-id-attribution.md. " +
            "Next slice should run a route-stabilized filtered HIR plus delayed body/block-time capture for 8227F1D8, keeping 8227FEE8 in the body-time filter as the parent comparator. If route-clean, choose the next default-off function/block-gated codegen experiment from the 8227F1D8 HIR and block body-time; if it black-idles, inspect last_global_owner_thread_id, owner_hint_source, and owner_hint_sys_tid before changing guest generated-code behavior."
        )
