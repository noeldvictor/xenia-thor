...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision notes: docs/research/20260525-203000-continual-harness-plan-reset.md, docs/research/20260525-203900-a64-mature-gap-reset-rerank.md, docs/research/20260526-012000-vmx128-route-stabilized-counters.md, docs/research/20260526-030200-82281d28-block-map-capture.md, docs/research/20260526-032000-82281d28-call-setup-audit.md, docs/research/20260526-033000-guest-call-arg-handoff-audit.md, docs/research/20260526-034500-guest-call-hir-coverage-capture.md, and docs/research/20260526-040500-a64-guest-call-fast-entry-feasibility.md. Use them as the active plan if present.",
        ...
        "Preferred next lanes are structural and currently ranked: default-off counter-only arm64_guest_call_fast_entry_audit skeleton for direct guest calls, with no alternate codegen and counters for eligibility, tail/indirect/extern/unresolved/debug/helper/host-call/stackpoint blockers, callee first-use coverage, normal-entry fallback pressure, estimated avoidable context store/load traffic, and dirty flush points; then guest-state cache design only if it avoids closed r1/r11/f1/fpscr/barrier lanes; then helper/block-linking only with a reusable static rule; then fastmem only with counter evidence; and GPU offload only when counters justify it. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
