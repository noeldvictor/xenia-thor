...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest call-boundary runtime audit: docs/research/20260524-085451-blue-dragon-call-boundary-state-runtime-audit.md. Default-off arm64_blue_dragon_call_boundary_state_audit now counts exact Blue Dragon 82282490 PC/offset sites and is wired through Android and tools/thor/thor_xenia_debug.ps1. Capture scratch/thor-debug/20260524-084805-* reached the visible opening sky/dragon-wing route with clean fatal-marker search on APK SHA 4E2A9728C1DDDCB4FD562843CB94CA4711EC96118992CDD44B79BCB49A40C36C. Runtime totals exactly match the static model: 675279 dynamic boundary hits, 13 candidate-dead stores per hit (dead=8778627) and 4 live-in stores per hit (live=2701116). This is audit proof only, not speed proof, because the counter instrumentation grows generated code. Next continuation should implement a separate default-off Blue-Dragon/function/PC-gated store-suppression probe for only the 13 candidate-dead sites, prove route safety first, and never skip the live-in r[3], f[1], fpscr, or lr stores. Keep arm64_blue_dragon_stvewx_stack_lane_fastpath and arm64_blue_dragon_mul_add_v128_fastpath default-off. " +
