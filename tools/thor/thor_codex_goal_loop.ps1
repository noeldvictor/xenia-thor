...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision notes: docs/research/20260525-203000-continual-harness-plan-reset.md, docs/research/20260525-203900-a64-mature-gap-reset-rerank.md, and docs/research/20260525-210145-vmx128-neon-family-audit.md. Use them as the active plan if present.",
        "Closed lanes: do not repeat f1/edge-payload behavior, stvewx, three-PC MUL_ADD_V128, fpscr, pre/post-promotion r1/r11, local-only 822824F0, or broad Vulkan work without fresh evidence. The edge-payload lifetime audit found segments_survived_no_kill=0 and f1_reads_before_kill=0, so do not materialize payload state or run a quiet speed A/B from that lane.",
        ...
        "Preferred next lanes are structural and currently ranked: pre-RA/register-allocation or guest-state-cache design, VMX128 PACK/UNPACK unit coverage or fresh PERMUTE/LOAD_VECTOR route-volume audit before behavior changes, helper ABI/block-linking audit, fastmem only with counter evidence, and GPU offload only when counters justify it.",
