...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision note: docs/research/20260527-123500-blue-dragon-surface-latency-capture-path.md. It adds automatic SurfaceFlinger latency capture for the game SurfaceView after the sampler capture showed no FPS proof, Main Thread still as the wall, Main Thread mostly on core 7, and gfxinfo insufficient for Vulkan SurfaceView pacing.",
        ...
        "Preferred next lane: FullDeploy current branch, then run a quiet 180-second Blue Dragon route with -Arm64OffsetMemoryAddressFastpath true, audit/disassembly/block tracing off, delayed body-time comparators 82282490,82281D28,82287788, -FrameCpuSampler true, and -FrameCpuSamplerAutoSurfaceLayer true. Run the proof summary and route-engine report after capture. Audit kernel/HLE churn only if present evidence aligns RtlEnterCriticalSection, RtlLeaveCriticalSection, KeRaiseIrqlToDpcLevel, or related helpers with missed frames. Keep -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, globally default-off. Do not use -AllowOrdinalFallback for behavior decisions.",
