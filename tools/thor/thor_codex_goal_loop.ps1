...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: hot-block codegen-floor/source audit with reliable one-function or file-backed HIR dumps, especially recovering 82281D28:8228233C; then guest-state cache design only if it avoids closed r1/r11/f1/fpscr/barrier lanes; then helper/block-linking only if the static audit proves a reusable direct-call or stackpoint rule beyond closed stack-sync and edge-payload lanes; then fastmem only with counter evidence; and GPU offload only when counters justify it. The 0x82282490 register-allocation audit found no allocator spill pressure; helper/block-link audit found final-interval dispatch_delta_total=4216370 with resolve_misses_total=0 and resolves_delta=5; the call-edge capture found 82282490 and 82281D28 dominated by self-recursive child body time, and the VMX128 route counters did not justify a broad vector behavior patch.",
