...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision notes: docs/research/20260525-203000-continual-harness-plan-reset.md, docs/research/20260525-203900-a64-mature-gap-reset-rerank.md, docs/research/20260526-012000-vmx128-route-stabilized-counters.md, docs/research/20260526-014000-82281d28-hir-block-profile-join.md, docs/research/20260526-015900-a64-block-profile-metadata-mapper.md, and docs/research/20260526-022000-82281d28-metadata-capture-blocker.md. Use them as the active plan if present.",
        ...
        "Preferred next lanes are structural and currently ranked: add file-backed or log-backed per-block HIR text/source spans for active unmappable 82281D28 rows, starting with block 20 / 8228233C-82282370, then rerun tools/thor/thor_hir_block_profile_join_audit.ps1; then guest-state cache design only if it avoids closed r1/r11/f1/fpscr/barrier lanes; then helper/block-linking only if static audit proves a reusable direct-call or stackpoint rule beyond closed stack-sync and edge-payload lanes; then fastmem only with counter evidence; and GPU offload only when counters justify it. Do not use -AllowOrdinalFallback for behavior decisions.",
