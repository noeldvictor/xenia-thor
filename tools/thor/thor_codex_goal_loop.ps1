...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260524-040404-blue-dragon-stvewx-lane-fastpath.md. The default-off A64 EXTRACT_I32 fastpath for only 82282580 -> lane 0 and 82282584 -> lane 1 is route-clean and audited: scratch/thor-debug/20260524-035227-* reached the visible opening sky/wing route, had clean fatal-marker search, and ended with fastpath=446272/1338814 fallback=0/0. Quiet same-APK A/B did not prove speed: control scratch/thor-debug/20260524-035623-* reached Microsoft Game Studios Presents, while fastpath-on scratch/thor-debug/20260524-035944-* reached an earlier sky/wing frame. Keep arm64_blue_dragon_stvewx_stack_lane_fastpath and audit default-off and do not repeat that exact A/B unchanged. Next target is larger body-backed work in 8228252C-822825C4, especially the 8228252C -> 82287788 child path or broader state/vector/FPR traffic. Previous lane provenance remains docs/research/20260524-034103-8228252c-stvewx-lane-provenance.md: 82282580 -> lane 0 from r1+0x50 and 82282584 -> lane 1 from r1+0x54 are proven under normal 16-byte PPC stack-pointer alignment, while 82282588 remains unknown because it uses r6+0x8. Previous codegen audit remains docs/research/20260524-033116-8228252c-a64-codegen-audit.md: the exact slice has body total 8530471, child edge 8228252C -> 82287788 body 5653971, approximate exclusive local work 2876500, and generated-code shape dominated by store_context:27, load_context:14, extract:9, splat:6, stvewx:3, and vmaddfp:3. Previous block-slice evidence remains docs/research/20260524-032005-82282490-dynamic-block-slice-report.md. Previous recursive-edge evidence remains docs/research/20260524-031017-82282490-exclusive-callee-attribution.md: direct-call edges account for 84.08% of final 82282490 body ticks, and 822825E0 -> 82282490 is mostly callee-body work, not generic call/prolog overhead. Previous entry/exit evidence remains docs/research/20260524-025544-a64-entry-exit-profiler.md: generated prolog/epilog/stackpoint overhead is tiny, so keep a64_enable_host_guest_stack_synchronization default true and treat stack-sync-off as code-size evidence only. " +
