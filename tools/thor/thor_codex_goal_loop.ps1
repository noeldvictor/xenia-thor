...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260524-021116-82282490-recursive-stack-sync-probe.md. Fresh route-clean block/call-edge captures under the Blue Dragon word-loop speed preset confirm the current wall is recursive 82282490 work, not stale 822824F0 vector work. Dynamic call-edge rows put 822825E0 -> 82282490 first with body_ticks_total=28602334, calls_total=117425, and peak ticks_per_call=1244; 822825C8 -> 8227FEE8 is secondary with body_ticks_total=11951162. Android/Thor tooling now exposes a64_enable_host_guest_stack_synchronization, but keep the Blue Dragon speed-pack default true. Same-APK stack-sync-off was route-clean and modestly improved 82282490 versus stack-sync-on: 27192906 -> 26462740 body_ticks_total and code size 87168 -> 85104, but this is not enough to disable a correctness guard globally. Next slice should run a route-matched stack-sync control sandwich or add stackpoint/prolog overhead attribution before changing the preset. " +
