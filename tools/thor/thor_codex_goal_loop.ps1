...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: same-APK quiet A/B or control sandwich on commit 1ed1dec66 / APK SHA 3195B62E66ABCE289187F6A0C4A67E621AE2A32931BBD98B790CD32CB0E33AB9 comparing -Arm64OffsetMemoryAddressFastpath false versus true with audit/disassembly/block tracing off and delayed body-time comparators 82282490,82281D28,82287788; guarded-stub / late-bound-entry design only if it explicitly handles normal-entry fallback, global indirection, stackpoint/debug/exception visibility, dirty flush points, and unresolved targets; guest-state cache only if it avoids closed r1/r11/f1/fpscr/barrier lanes; helper/block-linking only with a reusable static rule; GPU offload only when counters justify it. Only claim speed if matched captures show route-wide movement beyond noise. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
