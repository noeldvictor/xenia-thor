...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: deterministic block-profile-to-HIR metadata/mapper tooling for 82281D28 before behavior work; then guest-state cache design only if it avoids closed r1/r11/f1/fpscr/barrier lanes; then helper/block-linking only if static audit proves a reusable direct-call or stackpoint rule beyond closed stack-sync and edge-payload lanes; then fastmem only with counter evidence; and GPU offload only when counters justify it. The next mapper/tooling slice should log or dump runtime ordinal, FindBlockGuestAddress, first SOURCE_OFFSET, first printable PPC comment, HIR label if any, and source span, and update analysis scripts to refuse unsafe ordinal fallback.",
