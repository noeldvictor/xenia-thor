...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest 821CE028 f[1] clobber audit: docs/research/20260524-102113-821ce028-f1-clobber-audit.md. The child call from 82287788 no longer needs to be treated as an unknown f[1] clobber for this narrow carrier lane. Running tools/thor/thor_hir_fpr_carrier_safety_audit.ps1 on 821CE028 offset 296 reports target_loads=0, target_stores=0, and decision=no_target_context_access_observed. The only calls are call_indirect.6 return paths at 821CE1CC and 821CE1D4; the hotpath report shows only f[0] context traffic, not f[1] or fpscr. Treat direct calls to 0x821CE028 as f[1]-preserving for the parent 82287788 carrier lane, but do not patch 821CE028 or call it a speed target. Next continuation should add a default-off 82287788 runtime carrier audit/probe that treats __savegprlr_28, __restgprlr_28, and direct calls to 0x821CE028 as f[1]-preserving, then counts dynamic replacement opportunities for all 10 static f[1] loads before any quiet A/B. " +
