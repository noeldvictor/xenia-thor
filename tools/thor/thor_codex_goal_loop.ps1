...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-131338-8227f1d8-filtered-hir-block-body-time.md. Route-clean capture scratch/thor-debug/20260523-130934-* reached visible opening on APK SHA 962D3086F4030D9BD5A9D46AF5E8DFA4A320A13BFCD14135B8B077AECDC31CC5 with a clean fatal-marker search and emitted warning-level 8227F1D8 OptHIR. Final 8227F1D8 row: body_ticks_total=4238549, body_ticks_delta=907049, entries_delta=9675, ticks_per_entry=93, code_size=28180. Internal block body-time is almost entirely entry block 8227F1D8: body_ticks_total=4152240, peak_delta=1764204, peak_ticks_per_entry=262. That block has two direct calls, 0x82490030 and 0x826BFC7C, so the current wall is still inclusive call work rather than a proven local peephole. " +
            "Previous parent evidence remains docs/research/20260523-124029-8227fee8-callee-call-edge-split.md and owner-attribution remains documented in docs/research/20260523-001018-a64-owner-thread-id-attribution.md. " +
            "Next slice should run a route-stabilized call-edge split for 8227F1D8 with delayed body-time for 8227F1D8,82490030,826BFC7C and -Arm64SpeedProfileCallEdgeFilter 8227F1D8. If route-clean, choose the next default-off function/block-gated codegen experiment from dynamic edge rows; if it black-idles, inspect last_global_owner_thread_id, owner_hint_source, and owner_hint_sys_tid before changing guest generated-code behavior."
        )
