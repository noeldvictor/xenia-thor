...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest MUL_ADD_V128 runtime-audit patch: docs/research/20260524-074442-blue-dragon-mul-add-v128-runtime-audit.md. The backend now has default-off cvar arm64_blue_dragon_mul_add_v128_audit, gated to Blue Dragon function 82282490 and PCs 82282568, 8228256C, and 82282570. It logs total/per-PC calls, FPCR switches, software flush-path eligibility, input/output denormal sightings, NaN-fixup entry and repaired lanes, and source/dest vector copies. tools/thor/thor_xenia_debug.ps1 exposes -Arm64BlueDragonMulAddV128Audit, and presets keep it default-off. NativeCore passed. Next continuation should run FullDeploy, then a route-stabilized delayed 82282490 body-time speed capture with -Arm64BlueDragonMulAddV128Audit true. Do not patch a fastpath until that route-clean audit proves the expensive paths are cold. " +
