...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-155012-82490030-control-black-idle.md. The no-disassembly delayed body-time control scratch/thor-debug/20260523-154626-* used the same APK SHA 962D3086F4030D9BD5A9D46AF5E8DFA4A320A13BFCD14135B8B077AECDC31CC5, no disassembly filter, delayed body-time for 8227F1D8,82490030, and arm64_context_promotion_gpr_livein_r1=false. It also black-idled before visible opening route progress: fatal-marker search was clean, final screenshot was black, no 8227F1D8/82490030 body rows landed, and entry_delta dropped from 3843047 to 189129 to 0 by 15:47:15 and stayed flat. Idle attribution repeated the zombie signature with last_global_owner_sys_tid=20126, last_global_owner_thread_id=F80002E8, owner_hint=hit, owner_hint_source=thread_id_or_handle, owner_hint_tid=00000016, owner_hint_handle=F80002E8, owner_hint_state=zombie, and suspect stale owner_hint_sys_tid=946810032. " +
            ...
            "Do not repeat the exact filtered 82490030 capture or the exact no-disassembly control unchanged, and do not patch generated code from this evidence. Next slice should improve zombie owner/native TID attribution, especially live /proc/<pid>/task/<last_global_owner_sys_tid> mapping, native thread name/state if available, and logging that separates the real native owner TID from stale XThread hint fields before changing generated-code behavior."
        )
