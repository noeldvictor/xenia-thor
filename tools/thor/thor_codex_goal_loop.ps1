...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Do not start the 822824F0 stvewx peephole from stale entry-count evidence. The call-path audit docs/research/20260522-173542-82282490-call-path-audit.md shows the charged wall is inclusive call work: 822825E0 -> 0x82282490 charged_body_total=34726883, and 822825C8 -> 0x8227FEE8 charged_body_total=3216407. The targeted 8227FEE8 dump attempt scratch/thor-debug/20260522-172738-* idled before the route and emitted no callee dump. The default-off call-edge profiler now exists: docs/research/20260522-181040-a64-call-edge-profiler.md, cvar arm64_speed_profile_call_edge_filter, and Thor flag -Arm64SpeedProfileCallEdgeFilter. The first capture found and fixed an x9 callee-address clobber. The follow-up control sandwich docs/research/20260522-183742-call-edge-control-sandwich.md is a negative result for the current edge-filter path: controls scratch/thor-debug/20260522-182318-* and scratch/thor-debug/20260522-183118-* reached opening and 82282490 body-time on the same APK, while edge capture scratch/thor-debug/20260522-182705-* with arm64_speed_profile_call_edge_filter=82282490 black-idled by 18:27:52 and emitted no dynamic edge rows. Keep the cvar default-off and do not rerun that exact capture unchanged. Next add a low-overhead compile/activation audit or make the profiler inert until the filtered function is actually compiled/reached, then prove route safety before using edge timing."
        )
