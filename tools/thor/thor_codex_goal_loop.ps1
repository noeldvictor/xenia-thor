...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: FullDeploy plus a route-clean Thor capture with -Arm64OffsetMemoryAddressFastpath true and delayed body-time comparators 82282490,82281D28,82287788; treat that as route-safety proof only and do not run a quiet speed A/B until the enabled path reaches visible opening sky / dragon-wing with clean fatal markers; guarded-stub / late-bound-entry design only if it explicitly handles normal-entry fallback, global indirection, stackpoint/debug/exception visibility, dirty flush points, and unresolved targets; guest-state cache only if it avoids closed r1/r11/f1/fpscr/barrier lanes; helper/block-linking only with a reusable static rule; GPU offload only when counters justify it. Do not run a quiet speed A/B from counter-only, source-audit, or route-safety-only evidence. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
