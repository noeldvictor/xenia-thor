...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lane: guarded-stub / late-bound-entry only as behavior-neutral source work. A64Function now has separate fast-entry storage plus an r3-r10/lr payload and dirty-flush contract. The dirty-flush protocol audit confirms metadata coverage but generated behavior is still absent: no payload population, dirty flush codegen, stub codegen, callsite guard, or fast-entry stackpoint resume. Next fast-entry slice must be a default-off stub skeleton or no-op codegen protocol helper while normal machine_code(), direct calls, and global indirection stay unchanged. Use -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but keep the cvar default-off globally. Helper/block-linking only with a reusable static rule; GPU offload only when counters justify it; Android controller/settings UX is separate playability work. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
