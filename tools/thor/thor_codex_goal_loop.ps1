...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: use -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but move away from fastmem/addressing unless the slice explicitly adds a no-wrap range counter or static range analysis. Guest-state cache now has a default-off post-promotion non-closed audit skeleton for r31,r30,r29,r28,r27. The next valid slice is FullDeploy plus a route-clean one-function capture, starting with 0x82281D28, using -Arm64GuestStateNonclosedCacheAudit true and delayed body-time comparators 82282490,82281D28,82287788. Do not patch guest-state cache behavior or run a quiet speed A/B until route-clean counters prove material post-promotion volume with manageable flush/spill pressure. Guarded-stub / late-bound-entry only if the slice explicitly adds separate fast-entry storage and payload/dirty-flush contracts with generated behavior unchanged; helper/block-linking only with a reusable static rule; GPU offload only when counters justify it. Do not rerun the offset-helper A/B, unchanged 82282490/82287788 codegen-floor capture, current no-wrap eligibility audit, unchanged guarded-stub source audit, unchanged nonclosed guest-state ranking audit, or unchanged nonclosed GPR cache skeleton work, and only claim full speed with the full-speed proof marker criteria. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
