...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: use -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but move away from fastmem/addressing unless the slice explicitly adds a no-wrap range counter or static range analysis. Guest-state cache now has a non-closed target set: r31,r30,r29,r28,r27. The next valid guest-state slice is a default-off post-promotion counter-only audit for those slots with no store elision, no load replacement, no payload materialization, explicit miss/flush reasons, and spill-pressure estimates. Guarded-stub / late-bound-entry only if the slice explicitly adds separate fast-entry storage and payload/dirty-flush contracts with generated behavior unchanged; helper/block-linking only with a reusable static rule; GPU offload only when counters justify it. Do not rerun the offset-helper A/B, unchanged 82282490/82287788 codegen-floor capture, current no-wrap eligibility audit, unchanged guarded-stub source audit, or unchanged nonclosed guest-state ranking audit, and only claim full speed with the full-speed proof marker criteria. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
