...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: make the guest-call fast-entry audit callee-aware before any behavior work, with target/blocker rows, known callee first-use classification, unresolved/helper/tail blockers, normal-entry fallback pressure, and dirty flush points; close the lane if that cannot produce body-weighted known first-use traffic with manageable flush pressure. Do not run another unchanged 0x82281D28 fast-entry capture and do not run a quiet speed A/B from counter-only evidence. Fallback lanes: guest-state cache only if it avoids closed r1/r11/f1/fpscr/barrier lanes; helper/block-linking only with a reusable static rule; fastmem only with counter evidence; GPU offload only when counters justify it. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
