...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260524-031017-82282490-exclusive-callee-attribution.md. Route-clean same-run block body-time plus call-edge timing scratch/thor-debug/20260524-030450-* shows dynamic direct-call edges account for 84969304 ticks, or 84.08% of final 82282490 body ticks. The self-recursive 822825E0 -> 82282490 edge alone is 72194274 ticks, about 71.44% of final 82282490 body time and 98.32% of the 822825E0 block body. Treat this as callee-body work, not generic call/prolog overhead. Do not rerun the exact block/call-edge capture unchanged; next map dynamic block-body guests such as 8228252C back to exact 82282490 HIR/PPC spans, then choose a default-off function/block-gated state-traffic or lowering experiment from body-backed local spans. Previous entry/exit evidence remains docs/research/20260524-025544-a64-entry-exit-profiler.md: generated prolog/epilog/stackpoint overhead is tiny, so keep a64_enable_host_guest_stack_synchronization default true and treat stack-sync-off as code-size evidence only. " +
