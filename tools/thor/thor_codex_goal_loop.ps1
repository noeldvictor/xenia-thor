...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision notes: docs/research/20260525-203000-continual-harness-plan-reset.md, docs/research/20260525-203900-a64-mature-gap-reset-rerank.md, docs/research/20260526-012000-vmx128-route-stabilized-counters.md, docs/research/20260526-030200-82281d28-block-map-capture.md, and docs/research/20260526-032000-82281d28-call-setup-audit.md. Use them as the active plan if present.",
        "Closed lanes: do not repeat f1/edge-payload behavior, stvewx, three-PC MUL_ADD_V128, fpscr, pre/post-promotion r1/r11, local-only 822824F0, broad Vulkan work, PACK/UNPACK VMX128 behavior, broad PERMUTE/LOAD_VECTOR_SHL/SHR behavior, unchanged HIR mapper captures, ordinal-fallback HIR behavior, local 82281D28:8228233C-82282370 store elision, one-off 826BF770 caller inlining, or unchanged call/setup audits without fresh body-dominant evidence. The edge-payload lifetime audit found segments_survived_no_kill=0 and f1_reads_before_kill=0. PACK/UNPACK route-volume found PACK absent and UNPACK zero weighted hot-block volume. The 20260526 VMX128 route counters found current vector volume either in closed local stvewx/vmaddfp shapes or smaller non-dominant 82287788 blocks. The 82281D28 call/setup audit found all r3-r10/lr stores live into callees and strict local promotion has zero safe wins.",
        ...
        "Preferred next lanes are structural and currently ranked: generic default-off A64 guest-call argument handoff audit for direct guest calls, measuring parent r3-r10/lr stores, callee first-use reloads, tail/indirect/extern blockers, stackpoint/longjmp constraints, normal-entry fallback, and estimated avoidable store/reload traffic; then guest-state cache design only if it avoids closed r1/r11/f1/fpscr/barrier lanes; then helper/block-linking only with a reusable static rule; then fastmem only with counter evidence; and GPU offload only when counters justify it. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
