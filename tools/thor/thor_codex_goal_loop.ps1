...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest 82282490 branch-loop aggregate audit: docs/research/20260524-121503-82282490-branch-loop-aggregate-audit.md. Re-running tools/thor/thor_hir_dynamic_block_slice_report.ps1 and tools/thor/thor_hir_branch_state_audit.ps1 across entry 82282490-822824B8, loop head 822824B8-822824E8, and loop tail 822825F4-82282600 closes the standalone branch/GPR lane. The branch loop 822824B8 plus 822825F4 totals 2133128 local-exclusive body ticks and four CR predicate stores, but only three fallthrough-only GPR reloads, all in 822824B8; nearby entry 82282490-822824B8 adds only 249802 ticks and no reloads. Do not patch CR store/barrier fusion or a narrow branch-local GPR carrier next. Next useful output is a higher-traffic CFG-aware/interprocedural state-carrier or dirty-state audit/design around 8228252C-822825C4 and the 82282490 -> 82287788 state round-trip, with exact call/barrier/exit/helper flush rules. " +
