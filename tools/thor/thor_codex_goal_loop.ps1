...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision notes: docs/research/20260526-075500-a64-offset-address-fastpath-quiet-ab.md, docs/research/20260526-083000-a64-no-wrap-memory-eligibility-audit.md, docs/research/20260526-092500-a64-nonclosed-gpr-cache-capture.md, docs/research/20260526-112900-a64-route-structural-ranking.md, docs/research/20260526-114200-a64-context-cr-branch-lowering-audit.md, and docs/research/20260526-115700-a64-scalar-context-load-store-lowering.md. Use them as the active plan if present.",
        ...
        "Preferred next lane: the scalar context load/store audit reports decision=close_scalar_context_load_store_behavior_for_current_route. Do not drill one level deeper in that lane. Switch to a different structural class, or first write a source-only broader CFG/static-superblock or alternate-entry design that explicitly solves guest-visible state, fallback, late-bound targets, stackpoint/debug/exception visibility, and dirty flushes before behavior. Do not patch behavior, run Thor speed A/B, add another fast-entry micro-audit, use host-pointer immediate fastmem without no-wrap proof, reopen closed VMX128/nonclosed-cache behavior, enable arm64_cr_compare_branch_across_context_barrier, enable arm64_cr_store_elide_for_fused_branch, fuse single-PC barriers, or repeat scalar context load/store behavior from this source audit. A64Function fast-entry source contracts exist but generated behavior still does not use them; fast-entry stays closed until guard emission, payload population, dirty flush, late-bound fallback, stackpoint resume, and debug/exception visibility are solved explicitly. Use -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but keep the cvar default-off globally. GPU offload only when counters justify it; Android controller/settings UX is separate playability work. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
