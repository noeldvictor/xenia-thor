...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision note: docs/research/20260527-131500-blue-dragon-surface-latency-route-capture.md. The route-clean SurfaceFlinger capture reached visible opening sky / dragon-wing with focused fatal_marker_count=0, but fps_marker_count=0 and sustained_30fps_proof=false. Corrected game SurfaceView latency shows 126 valid presents over 55.150s, average interval 441.196ms, max 2361.047ms, all intervals over 33ms/50ms, and one rejected sentinel row. Main Thread / A64 generated code remains the wall; aggregate latency alone does not justify a kernel/HLE behavior patch.",
        ...
        "Preferred next lane: add a time-aligned present/jank attribution join, or a Perfetto / FrameTimeline path if SurfaceFlinger latency is insufficient. The join must determine whether long present gaps align with RtlEnterCriticalSection, RtlLeaveCriticalSection, KeRaiseIrqlToDpcLevel, XMA churn, A64 wall bursts, or Vulkan frame pacing before any behavior work. Do not rerun the unchanged 20260527-125840 route capture, do not reopen closed A64 micro-lanes, and do not patch kernel/HLE from aggregate latency evidence alone. Keep -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, globally default-off. Do not use -AllowOrdinalFallback for behavior decisions.",
