...
function New-AndroidUsabilityPrompt {
    $parts = @(
        ...
        "First read AGENTS.md, .agents/skills/xenia-windows-powershell-command-hygiene/SKILL.md, .agents/skills/xenia-thor-remote-debug/SKILL.md, the AYN Thor/Xenia debug skill, docs/worklogs/20260527.md or newer, docs/research/20260527-190000-android-remote-debug-test-rig.md, and scratch/thor-debug/codex-goal-loop.json if present.",
        ...
        "Current product priority: make the Android APK usable on AYN Thor. Use the actual app picker for UX/compat claims. When the user reports a fresh visual issue or crash, capture current screen/log first with tools/thor/thor_android_remote_debug.ps1.",
        ...
        "Known state: Project Sylpheed title/menu geometry, Android reenter abort, pthread_join abort, and XMA decoder abort have fixes/proofs. Android now has launcher-path AAudio driver-open proof; by-ear audible-output confirmation still needs the user present. Project Sylpheed still exposes heap release failures and a live black-frame/loading state without a scoped crash marker; free-site flow join maps the guest suballocator table but does not justify hiding or rounding physical frees. Recent-game status, controller remap, Back OSD, FPS badge, Exit game, and internal resolution settings exist.",
        "Preferred next product slice: continue Project Sylpheed late wait/progress PC mapping from docs/research/20260528-181500-project-sylpheed-free-site-flow-join.md, run a delayed/thread-filtered wait-trace capture, confirm audible output by ear with the user present, or handle another user-reported Android app usability bug.",
