...
function New-AndroidUsabilityPrompt {
    $parts = @(
        ...
        "Known state: Project Sylpheed title/menu geometry is user-confirmed fixed; do not reopen that lane without fresh title/menu evidence. Remaining Sylpheed class is guest heap / RtlRaiseException or black-frame compatibility. Recent-game status, controller remap, Back OSD, FPS badge, Exit to menu, and internal resolution settings exist.",
        "Preferred next product slice: Project Sylpheed guest heap / RtlRaiseException audit, black-frame compatibility, richer recent-game crash/stall classification, or another user-reported Android app usability bug.",
