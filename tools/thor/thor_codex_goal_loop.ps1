...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision notes: docs/research/20260525-203000-continual-harness-plan-reset.md, docs/research/20260525-203900-a64-mature-gap-reset-rerank.md, docs/research/20260526-012000-vmx128-route-stabilized-counters.md, docs/research/20260526-023500-hir-block-profile-stamps.md, and docs/research/20260526-030200-82281d28-block-map-capture.md. Use them as the active plan if present.",
        "Closed lanes: do not repeat f1/edge-payload behavior, stvewx, three-PC MUL_ADD_V128, fpscr, pre/post-promotion r1/r11, local-only 822824F0, broad Vulkan work, PACK/UNPACK VMX128 behavior, broad PERMUTE/LOAD_VECTOR_SHL/SHR behavior, unchanged HIR mapper captures, or ordinal-fallback HIR behavior without fresh body-dominant evidence. The edge-payload lifetime audit found segments_survived_no_kill=0 and f1_reads_before_kill=0, so do not materialize payload state or run a quiet speed A/B from that lane. PACK/UNPACK route-volume found PACK absent and UNPACK zero weighted hot-block volume. The 20260526 VMX128 route counters found current vector volume either in closed local stvewx/vmaddfp shapes or in smaller non-dominant 82287788 blocks. The 82281D28 mapper capture now maps the hot block with metadata/hir_block fields; do not rerun it unchanged or use -AllowOrdinalFallback for behavior decisions.",
        ...
        "Preferred next lanes are structural and currently ranked: offline/source audit of the mapped 82281D28:8228233C-82282370 call/setup block, especially r3-r10/lr context stores around 0x826BF770 and recursive 0x82281D28, to prove or reject a reusable helper ABI, direct-call, or stack-argument rule; then guest-state cache design only if it avoids closed r1/r11/f1/fpscr/barrier lanes; then helper/block-linking only with a reusable static rule; then fastmem only with counter evidence; and GPU offload only when counters justify it. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
