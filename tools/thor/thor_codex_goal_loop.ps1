...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Current priority, superseding the older chronology below: docs/research/20260525-170307-arm64-register-cache-residual-audit-capture.md records FullDeploy plus route-clean Thor residual audit capture for 0x82282490 on commit 85c422d2b, APK SHA 18097BA09F0C50596DED67D4251E4401FC7D458AA5EF7AAC7365B362D14CD19A. It reached the visible opening sky / dragon-wing route with clean fatal-marker search. Residual counters close the r1/r11 behavior lane: candidate_loads=174, candidate_stores=139, clean_hits_possible=0, dirty_hits_possible=0, normal_fallback=313, r1=107/11/0/0/118, r11=67/128/0/0/195, payload_materializations_allowed=0 behavior_changed=0. Do not implement stale same-block r1 clean-load replacement, r11 dirty caching, store elision, or quiet speed A/B from this lane. Next useful slice is caller-local or side-table edge-variant payload storage for 82282490:82282598 -> 82287788 with normal-entry fallback and counter-only payload/materialization diagnostics first. Final evidence remains CPU/JIT-heavy: 82282490=29927708, 82281D28=7155788, 82287788=3162326, Main Thread 96.1%, GPU Commands 11.5%; do not pivot to broad Vulkan work. Hardware acceleration menu docs/research/20260525-171305-thor-hardware-acceleration-menu.md says use Thor NEON/dot/I8MM/CRC/LSE only with opcode/source/counter proof and move only Xenos-like bulk graphics work to Adreno after a measured graphics-work audit finds CPU-side graphics cost. " +
