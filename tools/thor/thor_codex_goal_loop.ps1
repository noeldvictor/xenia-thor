...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: default-off A64 offset-aware address helper prototype or source-tested codegen audit for normal non-MMIO constant-offset LOAD_OFFSET / STORE_OFFSET paths, modeled on x64 ComputeMemoryAddressOffset while preserving x0 as the final 32-bit guest address, byte swap, MMIO / exception visibility, store-watch expectations, allocation_granularity threshold semantics, and fallback behavior; guarded-stub / late-bound-entry design only if it explicitly handles normal-entry fallback, global indirection, stackpoint/debug/exception visibility, dirty flush points, and unresolved targets; guest-state cache only if it avoids closed r1/r11/f1/fpscr/barrier lanes; helper/block-linking only with a reusable static rule; GPU offload only when counters justify it. Do not run a quiet speed A/B from counter-only or source-audit evidence. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
