...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: FullDeploy plus route-clean 0x82281D28 arm64_guest_call_fast_entry_audit capture with -Arm64GuestCallFastEntryAudit true -Arm64GuestCallFastEntryAuditFunction 0x82281D28 -Arm64GuestCallFastEntryAuditBudget 16 and delayed body-time comparators 82282490,82281D28,82287788; then analyze whether eligible guarded direct-call volume and dirty flush/fallback pressure justify a separate-entry design. Do not run a quiet speed A/B from the counter-only skeleton. Fallback lanes: guest-state cache only if it avoids closed r1/r11/f1/fpscr/barrier lanes; helper/block-linking only with a reusable static rule; fastmem only with counter evidence; GPU offload only when counters justify it. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
