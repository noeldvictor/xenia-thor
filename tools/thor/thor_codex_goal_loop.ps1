...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "The 82282490 vector-shape audit found stvewx=6, stvewx_dynamic_extract=6, extract=22, extract_dynamic=6, splat=152, permute=381, load_vector_shl=73, and load_vector_shr=64. 822824F0 is the only dynamic-hot vector block in the old entry-count profile and carries three stvewx dynamic extract/store shapes. " +
            "Next leave lower-noise per-block body-time attribution for 82282490, separating 822824F0 from the other entry-hot control blocks, a committed patch, or a concrete blocker. Do not start a stvewx/extract/splat peephole from entry counts alone; if 822824F0 remains body-time hot, make the first codegen experiment default-off and function/block-gated."
        )
