...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest call-boundary state audit: docs/research/20260524-083644-8228252c-call-boundary-state-audit.md. New tool tools/thor/thor_hir_call_boundary_state_audit.ps1 compares parent pre-call stores, callee first context access, and parent post-call context access. For 82282490 span 8228252C-822825C4 at direct call 82282598 -> 82287788, it found 17 pre-call store_context rows, about 188 approximate state bytes. Only 4 are live into the child (r[3], f[1], fpscr, lr); 13, about 160 approximate state bytes, are callee_dead_parent_dead_linear in the linear HIR audit, mostly VMX state. This is not proof that stores can be skipped. Next continuation should add a default-off function/span/call-gated state-store suppression audit/counter for those candidate sites before any behavior change. Keep arm64_blue_dragon_stvewx_stack_lane_fastpath and arm64_blue_dragon_mul_add_v128_fastpath default-off. " +
