...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-232053-global-critical-owner-source-attribution.md. A diagnostic patch added global critical-region owner sequence, owner age, and owner source fields to the A64 idle skip line, and tags Acquire, AcquireDirect, TryAcquire, XThread::LockApc, and ObjectTable::LookupObject. NativeCore and FullDeploy passed; APK SHA D68ED877D6C265420CD6853AB6C108C18F17E23911C2A1135016797D345C4823. Short validation capture scratch/thor-debug/20260523-231636-* stayed active for 100 seconds with clean fatal-marker search and no A64 idle-snapshot skip line, but ended at the loading overlay instead of visible opening. This proves no early diagnostic regression but does not yet exercise global_lock_owner_seq/global_lock_owner_age_ms/global_lock_owner_source. " +
            "Previous owner-native-TID evidence remains docs/research/20260523-160650-a64-owner-native-tid-attribution.md: capture scratch/thor-debug/20260523-160357-* reproduced the black-idle and proved the owner hint now uses real Linux gettid, while the matched native task was already gone and the XThread was zombie. " +
            ...
            "Do not repeat the exact filtered 82490030 capture or no-disassembly control unchanged, and do not patch generated code from this evidence. Next slice should run a longer route-stabilized attribution capture on the patched APK. If it black-idles, inspect global_lock_owner_seq, global_lock_owner_age_ms, global_lock_owner_source, and native-owner liveness. If it reaches the visible route and stays active, resume measured hotpath profiling from route-clean evidence instead of repeating stale 82490030 captures."
        )
