...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Parallel Vulkan design lane: docs/research/20260524-165737-mature-vulkan-port-speed-design.md and docs/research/20260524-171203-vulkan-counter-surface-audit.md. External emulator evidence from Dolphin, RPCS3, PCSX2, Khronos, Android, and mobile GPU guidance says mature Vulkan ports win by measuring GPU costs directly: pipeline cache misses, shader/pipeline compile stalls, barriers, render passes, readbacks, texture uploads/copies, descriptor churn, present waits, and frame pacing. New tool tools/thor/thor_vulkan_counter_surface_audit.ps1 maps repo-local surfaces for pipeline creation, queue submit/present, completion waits, barriers, render passes, copies/uploads/readbacks, descriptors, resolves, and GPU milestones; all planned surfaces are present and the safe next GPU patch is a default-off counter skeleton plus Thor route metadata. Do not pivot Blue Dragon away from A64 while Thor captures show Main Thread as the wall, and do not start RenderDoc-only FPS claims, Swappy/frame pacing, or broad Vulkan rewrites before these counters expose a GPU wall. " +
