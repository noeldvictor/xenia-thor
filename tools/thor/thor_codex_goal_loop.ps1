...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest Blue Dragon state-carrier audit capture: docs/research/20260524-141502-state-carrier-audit-route-capture.md. The first Thor capture for arm64_blue_dragon_state_carrier_design_audit reached the visible opening sky/dragon-wing route on APK SHA 5A80DD15AF4471FABE16E7370D6CA5EB25FC8582993F0208BE98DB33EFCFA2FD with a clean fatal-marker search, but log_level=0 suppressed the warning/body rows: no state-carrier audit row and no 82282490,82287788 body-time rows landed. tools/thor/thor_xenia_debug.ps1 now auto-lifts Blue Dragon speed captures to LogLevel=1 when body-time, block/call-edge, thread-snapshot, or audit rows are requested. Do not patch behavior from the no-row capture and do not rerun the exact same command unchanged. Next useful output is the same route-stabilized audit with the fixed script default or explicit -LogLevel 1, -Arm64BlueDragonStateCarrierDesignAudit true, -Arm64SpeedProfileBodyTimeFilter '82282490,82287788', and -Arm64SpeedProfileBodyTimeAfterMs 120000. Do not judge FPS from that audit capture, do not enable the probe in presets, and do not patch behavior until the counter row proves which carrier lane has enough dynamic payoff. " +
