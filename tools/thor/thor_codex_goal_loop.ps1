...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision notes: docs/research/20260525-203000-continual-harness-plan-reset.md, docs/research/20260525-203900-a64-mature-gap-reset-rerank.md, docs/research/20260526-012000-vmx128-route-stabilized-counters.md, docs/research/20260526-030200-82281d28-block-map-capture.md, docs/research/20260526-032000-82281d28-call-setup-audit.md, docs/research/20260526-033000-guest-call-arg-handoff-audit.md, and docs/research/20260526-034500-guest-call-hir-coverage-capture.md. Use them as the active plan if present.",
        "Closed lanes: do not repeat f1/edge-payload behavior, stvewx, three-PC MUL_ADD_V128, fpscr, pre/post-promotion r1/r11, local-only 822824F0, broad Vulkan work, PACK/UNPACK VMX128 behavior, broad PERMUTE/LOAD_VECTOR_SHL/SHR behavior, unchanged HIR mapper captures, ordinal-fallback HIR behavior, local 82281D28:8228233C-82282370 store elision, one-off 826BF770 caller inlining, unchanged call/setup audits, or guest-call fast-entry behavior from the single 82281D28 sample. The edge-payload lifetime audit found segments_survived_no_kill=0 and f1_reads_before_kill=0. PACK/UNPACK route-volume found PACK absent and UNPACK zero weighted hot-block volume. The 20260526 VMX128 route counters found current vector volume either in closed local stvewx/vmaddfp shapes or smaller non-dominant 82287788 blocks. The 82281D28 call/setup audit found all r3-r10/lr stores live into callees and strict local promotion has zero safe wins. The targeted guest-call coverage capture improved the generic audit to direct_ppc_bl_calls=81, argument_store_fields=439, callee_first_load_stores=247, callee_hir_missing_stores=126, normal_entry_fallback_required=81; this proves material ABI round-trip traffic but not a behavior patch yet.",
        ...
        "Preferred next lanes are structural and currently ranked: source-level fast-entry feasibility audit for A64 direct guest calls, function entry conventions, stackpoint/longjmp handling, debug/exception visibility, and normal-entry fallback before any runtime skeleton; then guest-state cache design only if it avoids closed r1/r11/f1/fpscr/barrier lanes; then helper/block-linking only with a reusable static rule; then fastmem only with counter evidence; and GPU offload only when counters justify it. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
