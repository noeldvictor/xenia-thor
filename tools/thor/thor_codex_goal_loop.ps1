...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Parallel Vulkan design lane: docs/research/20260524-165737-mature-vulkan-port-speed-design.md, docs/research/20260524-171203-vulkan-counter-surface-audit.md, and docs/research/20260524-173639-vulkan-perf-counter-skeleton.md. External emulator evidence from Dolphin, RPCS3, PCSX2, Khronos, Android, and mobile GPU guidance says mature Vulkan ports win by measuring GPU costs directly: pipeline cache misses, shader/pipeline compile stalls, barriers, render passes, readbacks, texture uploads/copies, descriptor churn, present waits, and frame pacing. tools/thor/thor_vulkan_counter_surface_audit.ps1 now reports recommended_next_patch=thor_route_capture_with_vulkan_trace_perf_counters and surfaces_missing=0. The default-off cvars vulkan_trace_perf_counters and vulkan_trace_perf_counters_log_interval are wired through Android extras, Thor metadata, IssueSwap, graphics pipeline create/cache-hit, command-processor submit, presenter acquire/submit/present, barriers, and render-pass begins. NativeCore and FullDeploy passed. Capture scratch/thor-debug/20260524-173312-* on APK SHA C1F35AF00D9CED398333E591326A4E69C7178CB20B27CCCB30C3D417BD6A91A6 reached the loading spinner with clean fatal-marker search and counter rows, including issue_swaps=710, pipeline_creates=44, queue_submits=710, barrier_submits=96807, present_acquires=711, and present_failures=0. Treat this as instrumentation proof only, not speed proof, because it did not reach opening/gameplay. Next GPU slice should be a route-stabilized 180-second control/counter sandwich with -VulkanTracePerfCounters true -VulkanTracePerfCountersLogInterval 120 before any GPU behavior patch. Do not pivot Blue Dragon away from A64 while Thor captures show Main Thread as the wall, and do not start RenderDoc-only FPS claims, Swappy/frame pacing, or broad Vulkan rewrites before these counters expose a GPU wall. " +
