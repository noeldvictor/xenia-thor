...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision notes: docs/research/20260525-203000-continual-harness-plan-reset.md, docs/research/20260525-203900-a64-mature-gap-reset-rerank.md, docs/research/20260525-233000-vmx128-route-volume-audit.md, docs/research/20260526-001500-a64-register-allocation-audit-capture.md, and docs/research/20260526-004800-a64-helper-block-link-audit.md. Use them as the active plan if present.",
        ...
        "Preferred next lanes are structural and currently ranked: Thor call-edge capture for 82282490/82281D28/82287788 using existing arm64_speed_profile_call_edge_filter support, current PERMUTE/LOAD_VECTOR_SHL/SHR route counters before VMX128 behavior, guest-state cache design only if it avoids closed r1/r11/f1/fpscr/barrier lanes, fastmem only with counter evidence, and GPU offload only when counters justify it. The 0x82282490 register-allocation audit found no allocator spill pressure, and the helper/block-link audit found final-interval dispatch_delta_total=4216370 with resolve_misses_total=0 and resolves_delta=5, so do not target resolver behavior next.",
