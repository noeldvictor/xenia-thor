...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: use -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, then continue fastmem/addressing only if the slice proves a broader reusable no-wrap / normal-memory eligibility rule for hot LOAD_OFFSET / STORE_OFFSET route blocks; otherwise close fastmem/addressing and move to a different A64 structural lane. Guarded-stub / late-bound-entry design only if it explicitly handles normal-entry fallback, global indirection, stackpoint/debug/exception visibility, dirty flush points, and unresolved targets; guest-state cache only if it avoids closed r1/r11/f1/fpscr/barrier lanes; helper/block-linking only with a reusable static rule; GPU offload only when counters justify it. Do not rerun the offset-helper A/B or unchanged 82282490/82287788 codegen-floor capture, and only claim full speed with the full-speed proof marker criteria. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
