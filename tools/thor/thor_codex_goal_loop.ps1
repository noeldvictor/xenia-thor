...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lane: route-engine evidence capture, not another behavior patch. Run the known Blue Dragon route with -Arm64OffsetMemoryAddressFastpath true, audit/disassembly/block tracing off, delayed body-time comparators 82282490,82281D28,82287788, -FrameCpuSampler true, and explicit FPS/proof parsing. Then run tools/thor/thor_blue_dragon_route_engine_reset_report.ps1 on the new log. Use the joined evidence to choose Android scheduling/core residency, kernel/HLE churn, region/early-exit quality, or real GPU/frame-pacing work. Do not rerun the old 20260526-124549 quiet capture unchanged and do not reopen current A64/VMX/fastmem/helper/GPU behavior lanes from it. Keep -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but keep the cvar default-off globally. Do not use -AllowOrdinalFallback for behavior decisions.",
