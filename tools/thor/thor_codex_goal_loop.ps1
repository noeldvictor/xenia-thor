...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest Blue Dragon state-carrier ABI audit: docs/research/20260524-163338-a64-state-carrier-abi-audit.md. New tool tools/thor/thor_a64_state_carrier_abi_audit.ps1 confirms the current A64 direct-call ABI is guest_return_only_in_x0, the existing f[1] carrier is only a callee-local 82287788 stack slot, and a broader 82282490 -> 82287788 payload is blocked without either an explicit call-ABI extension or an edge-specific compiled entry/variant with normal-entry fallback. Do not run another quiet speed A/B or patch generated behavior from the current state-carrier evidence. Next useful slice should add edge-specific ABI/variant design counters, or prove exact fpscr CFG writeback/flush rules before any generated behavior change. " +
