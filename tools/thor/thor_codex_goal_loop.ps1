...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-001018-a64-owner-thread-id-attribution.md. The first delayed 8227FEE8 block body-time capture scratch/thor-debug/20260522-235449-* black-idled before target rows: black screenshot, clean fatal search, no 8227FEE8 body/block rows after activation, and idle skip last_global_owner_sys_tid=14186 owner_hint=miss. A diagnostic-only patch now records last_global_owner_thread_id from xe::threading::current_thread_id(), lets Processor query the lock-free hint cache by guest thread ID or handle, and logs owner_hint_source plus owner_hint_sys_tid. NativeCore and FullDeploy passed; patched APK SHA 962D3086F4030D9BD5A9D46AF5E8DFA4A320A13BFCD14135B8B077AECDC31CC5. Short validation scratch/thor-debug/20260523-000506-* stayed active at loading with clean fatal search, so the new idle fields still need a black-idle capture. " +
            "Previous route-clean filtered HIR evidence remains docs/research/20260522-234847-8227fee8-filtered-hir-sandwich.md: 8227FEE8 is secondary behind 82282490 and HIR is state-traffic heavy with static lvlx/lvrx/stvlx/stvrx vector candidates in 82280A68 and 82280E1C, but no block body-time proof. " +
            "Next slice should repeat delayed 8227FEE8 block body-time on the patched APK with -Arm64SpeedProfileBodyTimeFilter 8227FEE8 -Arm64SpeedProfileBodyTimeAfterMs 120000 -Arm64SpeedProfileBlockFilter 8227FEE8 -Arm64SpeedProfileBlockBodyTime true. If route-clean, choose the next default-off function/block-gated codegen experiment from real block body-time. If it black-idles, inspect last_global_owner_thread_id, owner_hint_source, and owner_hint_sys_tid before changing guest generated-code behavior."
        )
