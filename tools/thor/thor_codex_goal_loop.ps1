...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-153235-8227f1d8-call-edge-split.md. Route-clean capture scratch/thor-debug/20260523-152754-* reached visible opening on APK SHA 962D3086F4030D9BD5A9D46AF5E8DFA4A320A13BFCD14135B8B077AECDC31CC5 with a clean fatal-marker search. Final dynamic rows show 82490030 almost fully explains 8227F1D8: parent 8227F1D8 body_ticks_total=4117139, child 82490030 body_ticks_total=4007328, and edge 8227F1D8 -> 82490030 body_ticks_total=4054641, calls_total=32107, ticks_per_call=72. 826BFC7C did not appear as a meaningful dynamic row in this route. Final perf remains CPU/JIT-heavy: Main Thread 103%, GPU Commands 3.8%, Draw Thread 0.0%; Android UI frame stats are not reliable game FPS. " +
            "Previous parent evidence remains docs/research/20260523-131338-8227f1d8-filtered-hir-block-body-time.md and owner-attribution remains documented in docs/research/20260523-001018-a64-owner-thread-id-attribution.md. " +
            "Next slice should run a route-stabilized filtered HIR plus delayed body/block-time capture for 82490030, keeping 8227F1D8 in the body-time filter as the parent comparator. Use -DisassembleFunctions true -DisassembleFunctionFilter 82490030 -Arm64SpeedProfileBodyTimeFilter 8227F1D8,82490030 -Arm64SpeedProfileBodyTimeAfterMs 120000 -Arm64SpeedProfileBlockFilter 82490030 -Arm64SpeedProfileBlockBodyTime true. If route-clean, choose the next default-off function/block-gated codegen experiment from 82490030 HIR and dynamic block rows; if it black-idles, inspect last_global_owner_thread_id, owner_hint_source, and owner_hint_sys_tid before changing guest generated-code behavior."
        )
