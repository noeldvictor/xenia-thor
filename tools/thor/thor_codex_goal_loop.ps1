...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest 8228252C state-carrier design audit: docs/research/20260524-133027-8228252c-state-carrier-design-audit.md. New tool tools/thor/thor_hir_state_carrier_design_audit.ps1 joins parent span HIR, callee HIR, and dynamic edge rows to print exact seed, kill, and writeback requirements for 82282490:8228252C-822825C4 and 82282598 -> 82287788. Current output classifies f[1] as a medium-risk read_only_entry_seed with loads=10, stores=0, and dynamic_load_upper=16912720; fpscr as a very-high-risk dirty_cfg_carrier with loads=26, stores=26, and writebacks at 82287ED4, 82287EDC, 82287EE4, and 82288220; r[3] as a high-risk mutable cache candidate; and lr as blocked call-link state. Do not patch behavior next or run a speed A/B from this directly. Next useful output is a default-off carrier-design counter/probe that reports dynamic seed hits, candidate reads, dirty writes, required writebacks, kills, and fallbacks before any behavior patch. " +
