...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Current decision note: docs/research/20260527-110500-blue-dragon-route-engine-sampler-capture.md. It completed the route sampler capture: no FPS proof, Main Thread remained the wall, Main Thread was mostly core 7, and gfxinfo was insufficient for Vulkan SurfaceView pacing.",
        ...
        "Preferred next lane: add route-clean FrameTimeline / SurfaceFlinger present-attribution capture support for the actual game SurfaceView layer before another Thor speed capture or behavior A/B. Then audit kernel/HLE churn only if present evidence aligns RtlEnterCriticalSection, RtlLeaveCriticalSection, KeRaiseIrqlToDpcLevel, or related helpers with missed frames. Keep -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, globally default-off. Do not use -AllowOrdinalFallback for behavior decisions.",
