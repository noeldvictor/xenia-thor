...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-160650-a64-owner-native-tid-attribution.md. A diagnostic patch added Processor::OnThreadNativeStarted to replace Android pthread_t-style thread hints with real Linux gettid values after XThread startup, and extended A64 idle snapshot skip logs with /proc/self/task liveness for owner and hint TIDs. NativeCore and FullDeploy passed; APK SHA 2E3D88F46BB709AA3A869634C24219FBBA0568695C1F4902693132701CF9EBE5. Short attribution capture scratch/thor-debug/20260523-160357-* reproduced the black-idle with clean fatal-marker search and final black screenshot. The new line says last_global_owner_sys_tid=28245, last_global_owner_thread_id=F80002EC, owner_hint=hit, owner_hint_source=system_tid, owner_hint_sys_tid=28245, owner_hint_tid=00000016, owner_hint_handle=F80002EC, owner_hint_state=zombie, native_owner_live=false, and owner_hint_native_live=false. " +
            ...
            "Do not repeat the exact filtered 82490030 capture or no-disassembly control unchanged, and do not patch generated code from this evidence. Next slice should instrument or audit global critical-region ownership/lifetime around the processor debug lock so a future black-idle distinguishes stale last-owner bookkeeping from a native thread exiting while holding the lock, recursive ownership into XThread exit/destruction, or another live owner that did not update the hint."
        )
