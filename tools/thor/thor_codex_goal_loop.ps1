...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lane: guarded-stub / late-bound-entry only as behavior-neutral source work. A64Function now has separate fast-entry storage, an r3-r10/lr payload and dirty-flush contract, a default-off stub skeleton cvar, a direct-call guard model, and a source-only payload/flush planner. The latest audit says payload_flush_plan_source_ready_behavior_blocked: the planner exists but generated code does not use it; normal machine_code(), direct calls, global indirection, payload population, dirty flush codegen, stub codegen, callsite guards, and stackpoint resume are unchanged. Next fast-entry work must be no-op codegen protocol helpers with behavior unchanged, or a lane switch; do not run Thor speed A/B or patch generated behavior from source-only evidence. Use -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but keep the cvar default-off globally. Helper/block-linking only with a reusable static rule; GPU offload only when counters justify it; Android controller/settings UX is separate playability work. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
