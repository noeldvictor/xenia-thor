...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: fastmem/addressing or A64 load/store codegen-floor counters for hot LOAD_OFFSET / STORE_OFFSET forms in 82281D28, with target block rows and body-time weighting before any behavior patch; guarded-stub / late-bound-entry design only if it explicitly handles normal-entry fallback, global indirection, stackpoint/debug/exception visibility, dirty flush points, and unresolved targets; guest-state cache only if it avoids closed r1/r11/f1/fpscr/barrier lanes; helper/block-linking only with a reusable static rule; GPU offload only when counters justify it. Do not run a quiet speed A/B from counter-only evidence. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
