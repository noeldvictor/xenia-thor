...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-154117-82490030-filtered-capture-black-idle.md. Capture scratch/thor-debug/20260523-153726-* used the intended 82490030 disassembly filter plus delayed 8227F1D8,82490030 body-time on APK SHA 962D3086F4030D9BD5A9D46AF5E8DFA4A320A13BFCD14135B8B077AECDC31CC5, but black-idled before opening route progress. Fatal-marker search was clean, no 82490030 HIR/body/block rows landed, final screenshot was black, and counters went flat by 15:38:15. Idle attribution reported a busy processor debug lock with last_global_owner_sys_tid=15739, last_global_owner_thread_id=F80002E8, owner_hint=hit, owner_hint_source=thread_id_or_handle, owner_hint_tid=00000016, owner_hint_handle=F80002E8, owner_hint_state=zombie. " +
            ...
            "Next slice should not repeat the exact filtered 82490030 capture unchanged and should not patch generated code. Run a no-disassembly delayed body-time control for 8227F1D8,82490030 on the same APK and route. If the control reaches visible opening and logs comparable body rows, rerun the filtered 82490030 HIR/block capture. If the control also black-idles, improve zombie owner/native TID attribution before changing generated-code behavior."
        )
