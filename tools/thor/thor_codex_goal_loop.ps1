...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-122718-8227fee8-block-body-time-route-clean.md. The repeated delayed 8227FEE8 block body-time capture scratch/thor-debug/20260523-122223-* reached the visible opening sky/wing route on APK SHA 962D3086F4030D9BD5A9D46AF5E8DFA4A320A13BFCD14135B8B077AECDC31CC5 with a clean fatal-marker search. Final 8227FEE8 row: body_ticks_total=6914659, body_ticks_delta=1871176, entries_delta=2041, ticks_per_entry=916, code_size=71868. Internal block body-time says the stale 82280A68/82280E1C vector peephole is not the next patch yet. The dynamic wall inside 8227FEE8 is call-heavy block 822809F4 with body_total=1937201, peak=665665, and direct calls to 0x8227F1D8 and 0x8247BE20. " +
            "Previous route-clean filtered HIR evidence remains docs/research/20260522-234847-8227fee8-filtered-hir-sandwich.md and the owner-attribution patch remains documented in docs/research/20260523-001018-a64-owner-thread-id-attribution.md. " +
            "Next slice should run a route-stabilized callee/call-edge capture for 8227FEE8,8227F1D8,8247BE20 with delayed body-time and -Arm64SpeedProfileCallEdgeFilter 8227FEE8. If route-clean, choose the next default-off function-gated codegen experiment from the callee split; if it black-idles, inspect last_global_owner_thread_id, owner_hint_source, and owner_hint_sys_tid before changing guest generated-code behavior."
        )
