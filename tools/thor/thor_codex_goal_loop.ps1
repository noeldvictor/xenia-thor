...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest evidence: docs/research/20260523-234203-processor-thread-lifecycle-owner-tags.md. A diagnostic patch added global_critical_region::Acquire(source) and tags Processor::OnThreadCreated, OnThreadNativeStarted, OnThreadExit, OnThreadDestroyed, OnThreadEnteringWait, and OnThreadLeavingWait. NativeCore and FullDeploy passed; APK SHA 862F86C44625B460A5BAB8528E25AB4E946F52CDB30137D7479D24AC3BD50FCB. Short validation capture scratch/thor-debug/20260523-233953-* stayed active for 100 seconds at the loading overlay with clean fatal-marker search and no A64 idle-snapshot skip line, so the new specific labels have not been exercised yet. " +
            "Previous owner-source evidence remains docs/research/20260523-232908-global-owner-source-black-idle.md: capture scratch/thor-debug/20260523-232432-* reproduced black-idle on commit 94fb4546e and showed global_lock_count=0, global_lock_owner_source='Acquire', global_lock_owner_age_ms=25053, dead native owner liveness, and zombie owner_hint_state. Previous owner-native-TID evidence remains docs/research/20260523-160650-a64-owner-native-tid-attribution.md: the owner hint now uses real Linux gettid, while the matched native task was already gone and the XThread was zombie. " +
            "Previous route-clean edge evidence remains docs/research/20260523-153235-8227f1d8-call-edge-split.md: 8227F1D8 -> 82490030 is the hot child edge, while 826BFC7C was not meaningful in that route. " +
            "Do not repeat the exact filtered 82490030 capture or no-disassembly control unchanged, and do not patch generated code from this evidence. Next slice should run a longer tagged-lifecycle attribution capture. If it black-idles, inspect the specific global_lock_owner_source before changing lock behavior. If it reaches the visible route and stays active, resume measured hotpath profiling from route-clean evidence instead of repeating stale 82490030 captures."
        )
