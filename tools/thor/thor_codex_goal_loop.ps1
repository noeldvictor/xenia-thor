...
switch ($Mode) {
    ...
    "EnableBlueDragonFullSpeed" {
        $prompt = (
            ...
            "Latest Blue Dragon state-carrier design probe: docs/research/20260524-135034-blue-dragon-state-carrier-design-probe.md. New default-off cvar arm64_blue_dragon_state_carrier_design_audit is counter-only and title/function/PC/offset gated to the audited 82282490 -> 82287788 path. It reports f[1] seed/read/helper-read/child-read/child-call/fallback and fpscr seed/read/dirty-write/required-writeback/call-kill/fallback counts without changing generated behavior. NativeCore and ApkShell builds passed; APK SHA after the build was 5A80DD15AF4471FABE16E7370D6CA5EB25FC8582993F0208BE98DB33EFCFA2FD. No Thor route capture has used this probe yet. Next useful output is a route-stabilized audit capture with -Arm64BlueDragonStateCarrierDesignAudit true, -Arm64SpeedProfileBodyTimeFilter '82282490,82287788', and -Arm64SpeedProfileBodyTimeAfterMs 120000. Do not judge FPS from that audit capture, do not enable the probe in presets, and do not patch behavior until the counter row proves which carrier lane has enough dynamic payoff. " +
