...
function New-AndroidUsabilityPrompt {
    $parts = @(
        ...
        "Known state: Project Sylpheed title/menu geometry, Android reenter abort, pthread_join abort, and XMA decoder abort have fixes/proofs. Android now has launcher-path AAudio driver-open proof; by-ear audible-output confirmation still needs the user present. Project Sylpheed still exposes heap release failures and a live black-frame/loading state without a scoped crash marker. Recent-game status, controller remap, Back OSD, FPS badge, Exit game, and internal resolution settings exist.",
        "Preferred next product slice: continue Project Sylpheed heap/free-site and wait/progress compatibility from docs/research/20260528-180300-project-sylpheed-library-wait-and-apk-native-guard.md, confirm audible output by ear with the user present, or handle another user-reported Android app usability bug.",
