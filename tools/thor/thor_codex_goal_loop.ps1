...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lane: switch away from the current fast-entry source-only chain, or first run a compile-time route-ranking audit that identifies a different body-dominant A64 backend class. A64Function has separate fast-entry storage, r3-r10/lr payload and dirty-flush contracts, guard/payload/codegen protocol helpers, and emitter planning points; latest decision is emitter_planning_source_ready_behavior_blocked. Generated behavior still does not use the protocol; normal machine_code(), direct calls, global indirection, payload population, dirty flush codegen, stub codegen, callsite guards, and stackpoint resume are unchanged. Do not add another fast-entry micro-audit, do not run Thor speed A/B, and do not patch generated fast-entry behavior unless a new slice solves guard emission, payload population, dirty flush, late-bound fallback, stackpoint resume, and debug/exception visibility explicitly. Use -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but keep the cvar default-off globally. Helper/block-linking only with a reusable static rule; GPU offload only when counters justify it; Android controller/settings UX is separate playability work. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
