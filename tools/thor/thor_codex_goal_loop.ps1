...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lanes are structural and currently ranked: hot-block A64 codegen-floor/disassembly audit for mapped body-dominant blocks, starting with 82281D28:8228233C-82282370, using deterministic block-profile mapping and tools/thor/thor_hir_a64_codegen_audit.ps1; source-level guarded-stub / late-bound-entry design only if it can explicitly handle normal-entry fallback, global indirection, stackpoint/debug/exception visibility, and dirty flush points for top body-weighted call targets; guest-state cache only if it avoids closed r1/r11/f1/fpscr/barrier lanes; helper/block-linking only with a reusable static rule; fastmem/addressing only when the codegen-floor or route counters point there; GPU offload only when counters justify it. Do not run a quiet speed A/B from counter-only evidence. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
