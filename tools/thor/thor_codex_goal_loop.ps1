...
function New-AndroidUsabilityPrompt {
    $parts = @(
        ...
        "Known state: Project Sylpheed title/menu geometry, Android reenter abort, pthread_join abort, and XMA decoder abort have fixes/proofs. Android now has native AAudio selection proof, but audible output still needs a route with AndroidAudioDriver opened. Direct-launch proof still exposes a guest heap / RtlRaiseException class. Recent-game status, controller remap, Back OSD, FPS badge, Exit to menu, and internal resolution settings exist.",
        "Preferred next product slice: prove audible Android audio through the game library, continue Project Sylpheed guest heap compatibility, or handle another user-reported Android app usability bug.",
