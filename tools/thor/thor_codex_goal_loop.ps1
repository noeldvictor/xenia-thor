...
function New-BlueDragonFullSpeedPrompt {
    $parts = @(
        ...
        "Preferred next lane: the static-superblock feasibility audit reports decision=static_superblock_behavior_blocked_pending_ir_level_design, and the route ranking now reports decision=body_dominant_a64_micro_lanes_closed_need_refiner_or_new_evidence. Before another A64 speed micro-audit, run tools/thor/thor_continual_harness_review.ps1 -Mode Window and rerank, or switch to a genuinely different structural class with fresh evidence. Do not patch behavior, run Thor speed A/B, add another fast-entry micro-audit, use host-pointer immediate fastmem without no-wrap proof, reopen closed VMX128/nonclosed-cache behavior, enable arm64_cr_compare_branch_across_context_barrier, enable arm64_cr_store_elide_for_fused_branch, fuse single-PC barriers, repeat scalar context load/store behavior, or patch multi-block/static-superblock behavior from this source audit. A64Function fast-entry source contracts exist but generated behavior still does not use them; fast-entry stays closed until guard emission, payload population, dirty flush, late-bound fallback, stackpoint resume, and debug/exception visibility are solved explicitly. Use -Arm64OffsetMemoryAddressFastpath true explicitly in future Blue Dragon speed captures, but keep the cvar default-off globally. GPU offload only when counters justify it; Android controller/settings UX is separate playability work. Large HIR captures should use file-backed output or adb -s c3ca0370 logcat -G 64M first. Do not use -AllowOrdinalFallback for behavior decisions.",
