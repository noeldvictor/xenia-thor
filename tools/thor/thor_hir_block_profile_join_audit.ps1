...
if (!$rows) {
    ...
    $rows | Select-Object -First $Top | ForEach-Object {
        Write-Output ("block={0} guest={1} total={2} delta={3} entries={4} tpe={5} hir_label_match={6} ppc_comment_match={7} ordinal_label={8} ordinal_match={9} nearest_hir_before={10} nearest_hir_after={11} nearest_ppc_before={12} nearest_ppc_after={13}" -f
            ...
            $_.nearest_ppc_comment_after)
    }
}

Write-Output ""
Write-Output "## Decision"
if ($unmatchedGuests -gt 0 -or $ordinalFallbackMismatches -gt 0) {
