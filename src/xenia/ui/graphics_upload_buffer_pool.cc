...
GraphicsUploadBufferPool::Page* GraphicsUploadBufferPool::Request(
  ...
  // Test against the current page's real capacity, not page_size_. A page
  // created before page_size_ grew is smaller than page_size_ claims, and
  // overshooting it writes past the end of its mapping into whatever follows.
  ...
  if (current_page_used_aligned + size > current_capacity || !writable_first_) {
    ...
    if (!writable_first_) {
      ...
      // built with. CreatePageImplementation (more specifically, the first
      // successful call) may grow page_size_, and this page does not grow
      // with it.
...
GraphicsUploadBufferPool::Page* GraphicsUploadBufferPool::RequestPartial(
  ...
  assert_true(xe::is_pow2(alignment));
  size = xe::align(size, alignment);
  // Same rule as Request: clamp against the open page's real capacity.
  const size_t current_capacity =
      writable_first_ ? writable_first_->capacity_ : page_size_;
