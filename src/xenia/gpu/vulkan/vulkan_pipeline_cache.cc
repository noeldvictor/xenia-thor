...
VulkanPipelineCache::GetCurrentPixelShaderModification(
  ...
  if (render_target_cache_.GetPath() ==
    ...
    // NOTE: gpu_foliage_lrz_force_depth sets depth-WRITE OFF at the pipeline
    // dynamic-state level (normalized_depth_control, NOT the register), for the
    // alpha-test foliage class on the host-RT path. So gate the early-Z force on
    // the SAME condition (cvar + alpha_test) - that's exactly the draws that will
    // have depth-write off, where forced EarlyFragmentTests is safe + rejects.
    bool force_early_z =
        cvars::gpu_foliage_force_early_z &&
        cvars::gpu_foliage_lrz_force_depth &&
    ...
    modification.pixel.rt0_blend_rgb_premult_src_alpha = 0;
    modification.pixel.rt0_blend_a_premult_src_alpha = 0;
    ...
    if (shader.writes_color_target(0)) {
      ...
      if ((blend_control.color_comb_fcn == xenos::BlendOp::kMin ||
        ...
        modification.pixel.rt0_blend_rgb_premult_src_alpha = 1;
      }
      ...
      if ((blend_control.alpha_comb_fcn == xenos::BlendOp::kMin ||
        ...
        modification.pixel.rt0_blend_a_premult_src_alpha = 1;
      }
    }
  }
  ...
  if (!edram_fragment_shader_interlock) {
        ...
        description.render_pass_key.depth_and_color_used >> 1;
    {
      ...
      bool rt0_rgb_premult =
          pixel_shader_modification.pixel.rt0_blend_rgb_premult_src_alpha != 0;
      bool rt0_a_premult =
          pixel_shader_modification.pixel.rt0_blend_a_premult_src_alpha != 0;
