...
bool VulkanRenderTargetCache::Initialize(uint32_t shared_memory_binding_count) {
  ...
  // Direct host resolve pipeline layouts (vulkan_direct_host_resolve):
  // set 0 = push-constants-slot uniform buffer (b1) transient layout,
  // set 1 = destination storage buffer, set 2 = source image(s).
...
VkPipeline VulkanRenderTargetCache::GetDirectHostResolvePipeline(
  ...
  VkPipeline& pipeline =
      direct_host_resolve_pipelines_[is_64bpp ? 1u : 0u][msaa_index]
                                    [scaled ? 1u : 0u][source_is_uint ? 1u : 0u];
...
bool VulkanRenderTargetCache::TryDirectHostResolveCopy(
  ...
  // Resolution scaling writes to the scaled resolve buffer, which needs
  // texture-cache APIs this port doesn't carry over (the Thor runs 1x1) -
  // fall back to the EDRAM dump path.
  ...
  if (resolve_is_depth) {
    ...
    if (resolve_color_format ==
      ...
      // The gamma encoding isn't handled by the direct resolve shaders.
      return false;
    }
  ...
  for (const ResolveCopyDumpRectangle& rect : dump_rectangles_) {
    ...
    if (resolve_is_depth) {
      ...
      // (This tree has no pixel-shader float24 conversion, so rounding is
      // purely the depth_float24_round_ choice.)
      if (depth_float24_round_) {
  ...
  const uint32_t height_scaled =
      ...
      scale_y;
  const uint32_t msaa_2x_sample_0 = draw_util::GetD3D10SampleIndexForGuest2xMSAA(
      0, msaa_2x_attachments_supported_);
  const uint32_t msaa_2x_sample_1 = draw_util::GetD3D10SampleIndexForGuest2xMSAA(
...
bool VulkanRenderTargetCache::Resolve(const Memory& memory,
  ...
  if (resolve_info.copy_dest_extent_length && !bd_drop_this_resolve) {
    ...
    if (GetPath() == Path::kHostRenderTargets &&
      ...
      // Direct host resolve (vulkan_direct_host_resolve): resolve the host
      // render targets straight to guest memory with compute shaders, skipping
      // both the EDRAM dump and the EDRAM->guest copy below. Falls back to the
      // dump path for any ineligible case.
      if (cvars::vulkan_direct_host_resolve) {
        draw_util::ResolveCopyShaderConstants dhr_copy_shader_constants;
        uint32_t dhr_group_count_x, dhr_group_count_y;
        draw_util::ResolveCopyShaderIndex dhr_copy_shader =
            resolve_info.GetCopyShader(
                draw_resolution_scale_x(), draw_resolution_scale_y(),
                dhr_copy_shader_constants, dhr_group_count_x,
                dhr_group_count_y);
        if (dhr_copy_shader != draw_util::ResolveCopyShaderIndex::kUnknown) {
          copied = TryDirectHostResolveCopy(
              resolve_info, dhr_copy_shader_constants, dhr_copy_shader,
              dump_base, dump_row_length_used, dump_rows, dump_pitch,
              shared_memory, texture_cache, written_address_out,
              written_length_out);
        }
      }
      if (!copied) {
        ...
        if (ShouldTraceVulkanEdramChecksum() &&
            !ReadbackEdramBufferRange(dump_base, dump_row_length_used,
                                      dump_rows, dump_pitch, resolve_info)) {
          return false;
        }
      }
    }
    ...
    if (!copied &&
      ...
      if (!copy_dest_committed) {
        ...
        // (passing the absolute destination byte offset via dest_base) instead
        // of allocating and writing a per-resolve descriptor. The scaled
        // resolve path passes dest_relative constants that assume a descriptor
        // based at the destination, so it keeps the transient descriptor.
            ...
            use_persistent_dest
                ...
                : command_processor_.AllocateSingleTransientDescriptor(
                      VulkanCommandProcessor::SingleTransientDescriptorLayout ::
                          kStorageBufferCompute);
        if (descriptor_set_dest != VK_NULL_HANDLE) {
          // Write the destination descriptor. With the persistent binding the
          // descriptor starts at the buffer origin, so the dest_base
          // adjustment below subtracts 0 and the push constant stays absolute.
          // TODO(Triang3l): Scaled resolve buffer binding.
          VkDescriptorBufferInfo write_descriptor_set_dest_buffer_info;
          write_descriptor_set_dest_buffer_info.buffer = shared_memory.buffer();
          write_descriptor_set_dest_buffer_info.offset =
              use_persistent_dest ? 0 : resolve_info.copy_dest_base;
          write_descriptor_set_dest_buffer_info.range =
          ...
          if (!use_persistent_dest) {
            ...
            dfn.vkUpdateDescriptorSets(device, 1, &write_descriptor_set_dest,
                                       0, nullptr);
          }
          ...
          if (draw_resolution_scaled) {
            ...
            // The descriptor is offset to the destination (or to the buffer
            // origin with the persistent binding), so make dest_base relative
            // to it.
