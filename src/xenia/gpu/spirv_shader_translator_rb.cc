  ...
  if (color_targets_written) {
    ...
    while (xe::bit_scan_forward(color_targets_remaining, &color_target_index)) {
      ...
      if (edram_fragment_shader_interlock_) {
        ...
        // issues in the host render target path. Skipped when the render
        // target cache stores gamma targets as linear UNORM16 and encodes at
        // the EDRAM boundaries itself.
        ...
        // Source blend factor pre-multiplication for MIN/MAX blending on the
        // host render target path (the factors are read from the shader
        // modification, chosen by the pipeline cache).
        Modification shader_modification = GetSpirvShaderModification();
        xenos::BlendFactor rt0_rgb_premult_factor =
            shader_modification.pixel.rt0_blend_rgb_premult_src_alpha
                ? xenos::BlendFactor::kSrcAlpha
                : xenos::BlendFactor::kOne;
        xenos::BlendFactor rt0_a_premult_factor =
            shader_modification.pixel.rt0_blend_a_premult_src_alpha
                ? xenos::BlendFactor::kSrcAlpha
                : xenos::BlendFactor::kOne;
