...
void SpirvShaderTranslator::ProcessTextureFetchInstruction(
  ...
  if (instr.opcode == ucode::FetchOpcode::kGetTextureGradients) {
    ...
    //      Unconditionally for LOD kTextureFetch: LOD and gradient exponent
    //      bias. Do not use this for result exponent adjustment.
    ...
    if (instr.opcode == ucode::FetchOpcode::kGetTextureWeights) {
      ...
      if (instr.opcode == ucode::FetchOpcode::kGetTextureComputedLod) {
        ...
        // Load fetch constant word 4, needed unconditionally for LOD biasing
        // and conditionally for stacked texture filtering.
        id_vector_temp_.clear();
        id_vector_temp_.push_back(const_int_0_);
        id_vector_temp_.push_back(builder_->makeIntConstant(
        ...
        if (instr.dimension == xenos::FetchOpDimension::k3DOrStacked) {
          ...
          // EDRAM tile-I/O lever) instead of store->DRAM->sample. Feeds the same
          // sample_result_unsigned/signed the swizzle/sign block below formats,
          // so the result is byte-exact with the sampled path for this fetch.
          // A Dim=SubpassData read uses the fragment's own location implicitly;
          // the SPIR-V coordinate operand is the canonical constant ivec2(0, 0).
          id_vector_temp_.clear();
        ...
        // word 3. Word 4 carries LOD bias in the same bit range.
        id_vector_temp_.clear();
        id_vector_temp_.push_back(const_int_0_);
        id_vector_temp_.push_back(builder_->makeIntConstant(
