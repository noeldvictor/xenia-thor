...
bool BaseHeap::AllocRange(uint32_t low_address, uint32_t high_address,
  ...
  *out_address = 0;
  ...
  uint32_t high_page_number =
      (high_address - heap_base_ + (page_size_ - 1)) / page_size_;
...
bool PhysicalHeap::TriggerCallbacks(
  ...
  if (!any_watched && !invalidate_unwatched) {
    ...
    // instruction. But ONLY when the guest page is actually writable:
    // returning true unconditionally (Edge does) turns a genuine AV on a
