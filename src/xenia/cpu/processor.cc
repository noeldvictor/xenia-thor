...
static const D3dHleSig kD3dHleSigs[] = {
    ...
    // --- Pipeline-rearch-to-host intercepts (RE 2026-07-04, docs/research/
    // pipeline-rearch-plan.md). IDENTIFIED at load (logged) but NOT auto-installed
    // yet - the native-render host bodies are the multi-session port. Types 4-7
    // fall through to normal declaration (dispatch only installs type 1). ---
    // EndTiling @0x8248A680: whole-scene field render + resolve (THE field intercept).
    {"XDK_D3D_EndTiling", 4,
     {0x7D, 0x88, 0x02, 0xA6, 0x48, 0x23, 0x55, 0xDD, 0xDB, 0xE1, 0xFF, 0xA0,
      0x94, 0x21, 0xFF, 0x20, 0x7C, 0x7F, 0x1B, 0x78, 0xFF, 0xE0, 0x08, 0x90,
      0x7C, 0x9A, 0x23, 0x78, 0x7C, 0xDD, 0x33, 0x78, 0x7C, 0xF9, 0x3B, 0x78,
      0x7D, 0x38, 0x4B, 0x78, 0x7D, 0x57, 0x53, 0x78, 0x3A, 0xC0, 0x00, 0x00}},
    // BeginTilingSetup @0x824884C8: tiling ring/worker setup.
    {"XDK_D3D_BeginTilingSetup", 5,
     {0x7D, 0x88, 0x02, 0xA6, 0x48, 0x23, 0x77, 0x91, 0x94, 0x21, 0xFF, 0x50,
      0x7C, 0x7E, 0x1B, 0x78, 0x81, 0x7E, 0x4F, 0xC0, 0x55, 0x6B, 0x05, 0xEF,
      0x40, 0x82, 0x01, 0x7C, 0x81, 0x7E, 0x34, 0xA4, 0x39, 0x5E, 0x2A, 0x7C,
      0x81, 0x3E, 0x34, 0xA8, 0x3B, 0x60, 0x00, 0x00, 0x3D, 0x00, 0xC0, 0x00}},
    // FlushRenderState @0x82488670: dirty-render-state flush + draw preamble.
    {"XDK_D3D_FlushRenderState", 6,
     {0x7D, 0x88, 0x02, 0xA6, 0x48, 0x23, 0x75, 0xE5, 0x94, 0x21, 0xFF, 0x40,
      0x7C, 0x7D, 0x1B, 0x78, 0xE8, 0x9D, 0x00, 0x10, 0x2B, 0x24, 0x00, 0x00,
      0x41, 0x9A, 0x00, 0x9C, 0x7C, 0x8B, 0x23, 0x78, 0x55, 0x6B, 0x05, 0xEE,
      0x2B, 0x2B, 0x00, 0x00, 0x41, 0x9A, 0x00, 0x1C, 0x38, 0xC0, 0x00, 0x00}},
    // DrawRects2D @0x82488C10: the only LIVE draw emitter (2D/HUD/composite rects).
    {"XDK_D3D_DrawRects2D", 7,
     {0x7D, 0x88, 0x02, 0xA6, 0x48, 0x23, 0x70, 0x4D, 0xDB, 0xC1, 0xFF, 0x98,
      0xDB, 0xE1, 0xFF, 0xA0, 0x94, 0x21, 0xFF, 0x20, 0x7C, 0x7F, 0x1B, 0x78,
      0xFF, 0xC0, 0x08, 0x90, 0x7C, 0x99, 0x23, 0x78, 0x7C, 0xB8, 0x2B, 0x78,
      0x7C, 0xDC, 0x33, 0x78, 0x7D, 0x1D, 0x43, 0x78, 0x81, 0x7F, 0x29, 0x40}},
};
...
Function* Processor::LookupFunction(Module* module, uint32_t address) {
  ...
  if (symbol_status == Symbol::Status::kNew) {
    ...
    // The title's statically linked C runtime (heap, memcpy, memset) is
    // planted as host code in XexModule::DeclareFunction (guest_crt_hooks.cc),
    // which every declare path passes through; a planted function arrives
    // here already kDeclared with its extern handler.
    if (function->behavior() == Function::Behavior::kExtern &&
        function->status() == Symbol::Status::kDeclared) {
      return function;
    }
