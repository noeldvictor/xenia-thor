...
// XenDroid ships this ON. It is default-OFF here pending a device A/B, per the
// standing rule: this rewrites guest control flow, so a detection mistake is a
// wrong-behaviour bug, not a slow one. Validate with the PPC suite (1481) and
// one uncapped title before flipping.
DEFINE_bool(
