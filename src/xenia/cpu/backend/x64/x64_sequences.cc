...
// - 213 -> $1 = $2 * $1 + $3
// - 231 -> $1 = $2 * $3 + $1
// PPC multiply-add NaN semantics, scalar double. Covers EVERY scalar form:
// fmadd/fmsub/fnmadd/fnmsub and their single-precision variants, which are
// emitted as F64 ops followed by a round-to-single Convert.
//
// Hardware returns the FIRST NaN operand in PPC's A, B, C order, QUIETED, with
// its sign LEFT ALONE - even for the negated forms. The HIR operands are
// (A, C, B), so the walk is src1, src3, src2. x64 had no fixup at all and took
// whatever the host FMA returned.
//
// ⚠ THE ANSWER IS BUILT BEFORE THE FMA, NOT AFTER, AND THAT IS NOT A STYLE
// CHOICE. Reading the operands afterwards is not possible here:
//   - dest commonly ALIASES a source (the sequences pick vfmadd213/231 for
//     exactly that reason), so the operand is gone once the result exists; and
//   - LoadConstantXmm materializes constants through STASH SLOT 0
//     (x64_emitter.cc: "Implies possible StashXmm(0, ...)!"), so stashing the
//     operands up front does not survive either - the sequence overwrites slot
//     0 when src3 is constant. That cost a full corpus run to find.
// Building the answer first sidesteps both: it reads the operands while they
// are still live and parks the result in xmm3, which these sequences never use.
template <typename ARGS>
static void EmitBuildPpcFmaNan_F64(X64Emitter& e, const ARGS& i) {
  auto operand = [&e](const auto& src, const Xmm& scratch) -> Xmm {
    if (src.is_constant) {
      e.LoadConstantXmm(scratch, src.constant());
      return scratch;
    }
    return src;
  };
  Xmm a = operand(i.src1, e.xmm0);   // A
  Xmm c = operand(i.src2, e.xmm1);   // C
  Xmm b = operand(i.src3, e.xmm2);   // B
  Xbyak::Label have;
  const Xmm order[3] = {a, b, c};    // PPC priority: A, B, C
  for (int step = 0; step < 3; ++step) {
    Xbyak::Label next;
    e.vucomisd(order[step], order[step]);
    e.jnp(next);
    e.vmovq(e.rax, order[step]);
    e.bts(e.rax, 51);                // quiet it; the SIGN is left alone
    e.vmovq(e.xmm3, e.rax);
    e.jmp(have, e.T_NEAR);
    e.L(next);
  }
  // No NaN operand: any NaN result is GENERATED by an invalid operation, and
  // PPC wants its own default rather than x86's.
...
struct MUL_ADD_F64
  ...
  static void Emit(X64Emitter& e, const EmitArgType& i) {
    ...
    // Only reached when the result is already NaN, so the hot path pays one
    // compare and a not-taken branch.
    {
      Xbyak::Label nf_done;
      e.vucomisd(i.dest, i.dest);
      e.jnp(nf_done);
      e.vmovsd(i.dest, e.xmm3);
      e.L(nf_done);
    }
  }
};
