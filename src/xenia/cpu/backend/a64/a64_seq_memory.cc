...
void TraceGuestStoreWatch(void* raw_context, uint32_t guest_pc,
  ...
  // NOTE: the guest-stack chain-walk was removed - it read guest stack memory that
  // can be unmapped mid-restore (fault storm). For a DIRECT write (BeginTiling
  // writes the rect value 0x2A0 straight to the tile state) lr IS the writer, so
  // no chain is needed; watch the rect word and read lr.
}
...
void EmitGuestStoreWatch(A64Emitter& e, const hir::Instr* instr,
  ...
  // NOTE: the inline range-check attempt (mov w16/cmp/b) caused a JIT fault storm
  // (per-store label/scratch reuse). Reverted to the plain per-store native call -
  // it's slow (can't reach BD's field via timed nav) but CORRECT, and the
  // save-state restore path (instant field) doesn't need it fast.
