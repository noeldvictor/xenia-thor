...
uint64_t ResolveFunction(void* raw_context, uint64_t target_address) {
  ...
  if (!code) {
    // RACE WITH THE AOT PRECOMPILE THREAD, not a missing function.
    //
    // Processor::DemandFunction only COMPILES when Module::DefineFunction
    // returns kNew. Its own comment: "If it's already being generated by
    // another thread this will block and return DECLARED." So when the
    // PrecompileJIT thread is mid-generation of exactly this function, we take
    // the not-kNew path, report SUCCESS, and hand back a Function whose
    // machine_code() is still null - and the resolve thunk then falls through
    // to brk(0xF000) and the process SIGTRAPs.
    //
    // That is why this only bites BIG titles: the AOT pass must still be
    // running when the guest starts making indirect calls. MagnaCarta 2 is
    // ~47.4k functions and the guest launches while ~249 are outstanding;
    // Gears is ~28.5k and traps at the IDENTICAL stub address.
    //
    // Re-ask a bounded number of times, yielding between attempts, so the
    // generating thread can publish. Bounded because a genuinely uncompilable
    // function must still fail loudly rather than hang a guest thread forever.
