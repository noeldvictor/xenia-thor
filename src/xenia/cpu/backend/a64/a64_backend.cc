...
DEFINE_bool(
    ...
    "Thor ARM64 speed lane: replace Blue Dragon's 82486178:8248627C 4x "
    "VMX load/store copy loop with a narrow block-gated A64 path. "
...
DEFINE_uint32(
    ...
    "a64");
DEFINE_bool(
    arm64_guest_call_fast_entry_audit, false,
    "Thor ARM64 speed lane: log compile-time direct guest-call fast-entry "
    "eligibility and blocker summaries. Counter-only; does not emit alternate "
    "entry code.",
    "a64");
DEFINE_uint32(
    arm64_guest_call_fast_entry_audit_function, 0,
    "Thor ARM64 speed lane: optional exact guest function start address for "
    "the guest-call fast-entry audit. 0 logs all functions.",
    "a64");
DEFINE_uint32(
    arm64_guest_call_fast_entry_audit_budget, 64,
...
void LogResolveFailure(const char* why, uint64_t target_address,
                       ppc::PPCContext* ctx, Function* fn = nullptr) {
  static std::atomic<uint32_t> count{0};
  uint32_t c = ++count;
  if (c > 16) {
    return;
  }
  // Report WHAT the function is, not just that it failed. status/behavior are
  // the discriminators: a kProlog/kEpilogReturn saverest helper legitimately
  // has no standalone code on a64 (the emitter INLINES those), which would make
  // an indirect call to one unresolvable BY DESIGN rather than by accident.
  XELOGE(
      "RESOLVE FAILED ({}): guest target={:08X} caller_lr={:08X} r1={:08X} "
      "status={} behavior={} is_guest={} range={:08X}-{:08X} "
      ...
      fn ? static_cast<int>(fn->status()) : -1,
      fn ? static_cast<int>(fn->behavior()) : -1, fn ? fn->is_guest() : false,
      fn ? fn->address() : 0, fn ? fn->end_address() : 0);
}
}  // namespace
...
uint64_t ResolveFunction(void* raw_context, uint64_t target_address) {
  ...
  if (!code) {
    // A function reported as SUCCESSFULLY resolved, with NO machine code.
    //
    // I first blamed a race against the AOT precompile thread. THAT IS WRONG,
    // and our own code refutes it - recorded here so nobody re-derives it:
    //   * Module::DefineSymbol SPINS while the symbol is kDefining ("still
    //     defining, so spin"), so a concurrent generator is waited for, not
    //     raced past.
    //   * Processor::DemandFunction DOES check kFailed and returns false, so a
    //     failed compile surfaces as the "no function" path, not this one.
    // So this state is neither a race nor a failed compile, and the cause is
    // still OPEN.
    //
    // The retry below is therefore a PROBE, not a fix: if a bounded yield-loop
    // ever recovers, the state was transient after all and the two arguments
    // above are incomplete; if it never recovers (expected), the log line
    // distinguishes that cleanly. Bounded so a genuinely uncompilable function
    // fails loudly instead of hanging a guest thread.
    for (int attempt = 0; attempt < 64 && !code; ++attempt) {
      xe::threading::MaybeYield();
      code = guest_fn->machine_code();
    }
    if (!code) {
      backend->RecordResolveFunction(false);
      // Genuinely never compiled - a discovery or compilation problem, not the
      // race above.
      LogResolveFailure("no machine code after retry", target_address,
                        guest_context, fn);
      return 0;
    }
  }
...
void A64Backend::StartSpeedProfiler() {
  ...
  if (cvars::arm64_guest_call_fast_entry_audit) {
    XELOGW("A64 guest-call fast-entry audit enabled: function={:08X} "
...
void A64Backend::LogSpeedProfile() {
  ...
  if (cvars::arm64_blue_dragon_edge_variant_audit) {
    ...
    for (uint32_t i = 0; i < kBlueDragonEdgeVariantF1ReadSiteCount; ++i) {
      active_f1_read_sites[i] = load_delta(
          blue_dragon_edge_variant_active_f1_read_site_counts_[i],
          last_blue_dragon_edge_variant_active_f1_read_site_counts_[i]);
    }
    std::pair<uint64_t, uint64_t> active_call_kill_sites
        [kBlueDragonEdgeVariantCallKillSiteCount];
    for (uint32_t i = 0; i < kBlueDragonEdgeVariantCallKillSiteCount; ++i) {
      active_call_kill_sites[i] = load_delta(
          blue_dragon_edge_variant_active_call_kill_site_counts_[i],
          last_blue_dragon_edge_variant_active_call_kill_site_counts_[i]);
    }
    XELOGW(
    ...
    XELOGW(
        ...
        active_f1_read_sites[3].first, active_f1_read_sites[4].second,
        active_f1_read_sites[4].first, active_f1_read_sites[5].second,
        active_f1_read_sites[5].first, active_f1_read_sites[6].second,
...
bool A64Backend::Initialize(Processor* processor) {
  ...
  XELOGI("A64Backend: clock-spin-yield {} (stride={}, sleep_us={}, window_us={})",
         ...
         uint32_t(cvars::a64_clock_spin_yield_window_us));
