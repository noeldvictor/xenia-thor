...
void A64Emitter::Call(const hir::Instr* instr, GuestFunction* function) {
  // (Removed 2026-08-07: four Blue Dragon audit blocks, 231 of this
  // function's 462 lines, all gated on cvars defaulting to FALSE and all
  // keyed to ONE hardcoded guest address, 0x82287788. A64Emitter::Call is
  // the hottest emitter entry point - it runs for every guest call site of
  // every one of ~28k AOT-compiled functions - and half of it was debris
  // from the BD native-renderer project the user cancelled. The cvars and
  // their counters still exist elsewhere; only the emit-time hooks are gone.)
