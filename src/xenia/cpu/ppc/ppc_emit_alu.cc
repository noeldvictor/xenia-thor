...
int InstrEmit_negx(PPCHIRBuilder& f, const InstrData& i) {
  // RT <- ¬(RA) + 1
  if (i.XO.OE) {
    // With XER update. OV is not tracked. Log it and emit the plain
    // negation, the same as the other OE forms in this file. The ISA result
    // for RA == 0x8000000000000000 is RA itself, which two's complement
    // negation also gives; only the OV bit is missing.
    XEINSTRNOTIMPLEMENTED();
  }
...
int InstrEmit_rldicx(PPCHIRBuilder& f, const InstrData& i) {
  ...
  // r <- ROTL64((RS), n)
  // b <- mb[5] || mb[0:4]
  // m <- MASK(b, ~n & 63)   ; clears the high bits above b AND the low bits
  //                           below (63 - n), i.e. clear-left-and-right
  // RA <- r & m
  uint32_t sh = (i.MD.SH5 << 5) | i.MD.SH;
  uint32_t mb = (i.MD.MB5 << 5) | i.MD.MB;
  uint64_t m = XEMASK(mb, 63 - sh);
