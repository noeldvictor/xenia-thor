...
bool MMIOHandler::TryDecodeLoadStore(const uint8_t* p,
  ...
  // No LDR + REV pair detection here. The x64 MOVBE case sets byte_swap
  // because MOVBE loads and swaps in one instruction and the handler skips
  // the whole instruction. On arm64 the a64 JIT emits LDR then REV as two
  // instructions. length is 4, so the handler resumes at LDR+4 and the REV
  // still executes. byte_swap stays false: the handler stores the
  // little-endian load result and the REV produces the guest value.
