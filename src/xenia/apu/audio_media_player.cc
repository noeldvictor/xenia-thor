...
// NOTE(kernel-port 2026-08): the host-side decode/mix worker is compiled out
// in this fork - it requires a newer FFmpeg (AVChannelLayout), AudioDriver
// Pause/Resume/SetVolume and the std::mutex alias, none of which exist here.
...
void AudioMediaPlayer::Setup() {
  ...
  // Guest scratch frame for driver submission (our AudioDriver consumes a
  // guest physical address - see WorkerThreadMain).
  sample_buffer_ptr_ = kernel_state_->memory()->SystemHeapAlloc(
      xe::apu::AudioDriver::kFrameSizeMax, 256, kSystemHeapPhysical);
...
void AudioMediaPlayer::Play() {
  ...
  /* driver_->SetVolume: not in this fork's AudioDriver */
...
void AudioMediaPlayer::Pause() {
  ...
  if (driver_) {
    /* driver_->Pause: not in this fork's AudioDriver */
  }
...
void AudioMediaPlayer::Stop(bool change_state, bool force) {
  ...
  if (driver_ && IsPaused()) {
    /* driver_->Resume: not in this fork's AudioDriver */
  }
...
void AudioMediaPlayer::ProcessAudioBuffer(std::vector<float>* buffer) {
  while (buffer->size() >= xe::apu::AudioDriver::kFrameSamplesMax) {
    ...
    // Our AudioDriver::SubmitFrame takes a GUEST physical pointer (the XMA
    // path), while Edge's takes host float samples. Stage the mixed frame
    // into the guest scratch buffer allocated in Setup() and submit that.
    if (sample_buffer_ptr_) {
      std::memcpy(kernel_state_->memory()->TranslatePhysical(sample_buffer_ptr_),
                  buffer->data(), xe::apu::AudioDriver::kFrameSizeMax);
      driver_->SubmitFrame(sample_buffer_ptr_);
    }
...
bool AudioMediaPlayer::SetupDriver(uint32_t sample_rate, uint32_t channels) {
  ...
  // NOTE(kernel-port 2026-08): Edge allocates a dedicated standalone
  // AudioDriver here (its AudioSystem::CreateDriver returns one for a
  // host-side mixer). This fork's AudioSystem::CreateDriver has a different
  // contract - it binds a driver to a GUEST client slot - so there is no
  // spare driver to hand the media player. Report failure: the XMP state
  // machine (playlists, state, next/prev, volume as reported to the guest)
  // stays fully functional and Play() simply produces no host audio.
  // Re-enabling = porting Edge's apu/audio_driver + audio_system split.
  XELOGW(
      "AudioMediaPlayer: host XMP playback unavailable in this build "
      "(sample_rate={}, channels={}); playlist state is still emulated",
...
void AudioMediaPlayer::DeleteDriver() {
  ...
  if (driver_) {
    ...
    /* driver_->Shutdown: not in this fork's AudioDriver */
    driver_.reset();
  }
}
