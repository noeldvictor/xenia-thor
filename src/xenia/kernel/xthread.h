...
class XThread : public XObject, public cpu::Thread {
 public:
  ...
  // a64 APC fastpath hooks (thor): a host-side count of queued APCs, published
  // into PPCContext as a64_apc_pending_count so the a64 KfLowerIrql inline
  // guard can skip the slow path when nothing is pending. Maintained by the
  // KeInsertQueueApc/KeRemoveQueueApc HLE; a stale nonzero value only costs a
 ...
 protected:
  ...
  // a64 APC fastpath (thor): queued-APC count read lock-free by the JIT via
  // a64_apc_pending_count_ptr(). See NoteApcQueued/NoteApcDequeued.
